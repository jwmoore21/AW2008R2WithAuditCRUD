/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[dbo].[uspDel_ErrorLogWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[uspDel_ErrorLogWithAudit]
GO
*/

CREATE PROCEDURE [dbo].[uspDel_ErrorLogWithAudit]
  @ErrorLogID INT,
...
================================================================================
Schema:      dbo
Object:      uspDel_ErrorLogWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_ErrorLogWithAudit' );

    /* ------------------------------------------------------------------------- */
    /* Table Primary Key(s) */
    DECLARE @_ErrorLogID  INT;
    SET @_ErrorLogID = @ErrorLogID;
    ...
    (
      ...
      WHERE
        [so].[type] = 'TR'
      AND USER_NAME([so2].[uid]) = 'dbo'
      AND OBJECT_NAME([so].[parent_obj]) = 'ErrorLog'
    ...
    BEGIN
      ...
      WHERE
        [ErrorLog].[ErrorLogID] = @_ErrorLogID
      ;
    ...
    BEGIN
      ...
      BEGIN
        ...
        OUTPUT
          ...
          [DELETED].[UserName],
          [DELETED].[ErrorNumber],
          [DELETED].[ErrorSeverity],
          [DELETED].[ErrorState],
          [DELETED].[ErrorProcedure],
          [DELETED].[ErrorLine],
    ...
    FROM
      [Audit].[AuditErrorLog] AS [AuditErrorLog] WITH(NOLOCK)
    WHERE
      [AuditErrorLog].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditErrorLog].[ErrorLogID]
          ,[AuditErrorLog].[ErrorTime]
          ,[AuditErrorLog].[UserName]
          ,[AuditErrorLog].[ErrorNumber]
          ,[AuditErrorLog].[ErrorSeverity]
          ,[AuditErrorLog].[ErrorState]
          ,[AuditErrorLog].[ErrorProcedure]
          ,[AuditErrorLog].[ErrorLine]
          ,[AuditErrorLog].[ErrorMessage]
        FROM
          [Audit].[AuditErrorLog] AS [AuditErrorLog] WITH(NOLOCK)
        WHERE
          [AuditErrorLog].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditErrorLog].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('ErrorLog'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
SET @ErrorLogID = FILL IN;

EXEC dbo.uspDel_ErrorLogWithAudit
...
SELECT AuditErrorLog.*
FROM   Audit.AuditErrorLog AS AuditErrorLog WITH(NOLOCK)
WHERE
  AuditErrorLog.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_ErrorLogWithAudit';
GO