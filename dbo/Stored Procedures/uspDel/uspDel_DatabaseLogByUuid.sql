/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[dbo].[uspDel_DatabaseLogByUuid]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[uspDel_DatabaseLogByUuid]
GO
*/

CREATE PROCEDURE [dbo].[uspDel_DatabaseLogByUuid]
...
================================================================================
Schema:      dbo
Object:      uspDel_DatabaseLogByUuid
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_DatabaseLogByUuid' );
    ...
    (
      ...
      AND [tables].[name] = 'AuditDatabaseLog'
    );
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'DatabaseLog'
    ...
    FROM
      [Audit].[AuditDatabaseLog] AS [AuditDatabaseLog] WITH(NOLOCK)
    WHERE
      [AuditDatabaseLog].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditDatabaseLog].[DatabaseLogID]
          ,[AuditDatabaseLog].[PostTime]
          ,[AuditDatabaseLog].[DatabaseUser]
          ,[AuditDatabaseLog].[Event]
          ,[AuditDatabaseLog].[Schema]
          ,[AuditDatabaseLog].[Object]
          ,[AuditDatabaseLog].[TSQL]
          ,[AuditDatabaseLog].[XmlEvent]
        FROM
          [Audit].[AuditDatabaseLog] AS [AuditDatabaseLog] WITH(NOLOCK)
        WHERE
          [AuditDatabaseLog].[AuditBatchId] = @_AuditBatchId;
...
EXEC dbo.uspDel_DatabaseLogByUuid
  @Uuid = @Uuid,
...
SELECT AuditDatabaseLog.*
FROM   Audit.AuditDatabaseLog AS AuditDatabaseLog WITH(NOLOCK)
WHERE
  AuditDatabaseLog.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_DatabaseLogByUuid';
GO