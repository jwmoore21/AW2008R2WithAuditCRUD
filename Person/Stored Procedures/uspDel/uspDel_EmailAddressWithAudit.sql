/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Person].[uspDel_EmailAddressWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Person].[uspDel_EmailAddressWithAudit]
GO
*/

CREATE PROCEDURE [Person].[uspDel_EmailAddressWithAudit]
...
================================================================================
Schema:      Person
Object:      uspDel_EmailAddressWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_EmailAddressWithAudit' );

    /* ------------------------------------------------------------------------- */
    /* Table Primary Key(s) */
    DECLARE @_BusinessEntityID  INT;
    DECLARE @_EmailAddressID  INT;
    SET @_BusinessEntityID = @BusinessEntityID;
    SET @_EmailAddressID = @EmailAddressID;
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'EmailAddress'
      ...
      (
        ...
        AND [tables].[name] = 'AuditEmailAddress'
      ) = 1
    ) = 1
    BEGIN
      ...
      WHERE
        [EmailAddress].[BusinessEntityID] = @_BusinessEntityID
        AND [EmailAddress].[EmailAddressID] = @_EmailAddressID
      ;
    ...
    FROM
      [Audit].[AuditEmailAddress] AS [AuditEmailAddress] WITH(NOLOCK)
    WHERE
      [AuditEmailAddress].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditEmailAddress].[BusinessEntityID]
          ,[AuditEmailAddress].[EmailAddressID]
          ,[AuditEmailAddress].[EmailAddress]
          ,[AuditEmailAddress].[rowguid]
          ,[AuditEmailAddress].[RowStatus]
          ,[AuditEmailAddress].[CreatedBy]
          ,[AuditEmailAddress].[ModifiedBy]
          ,[AuditEmailAddress].[CreatedDate]
          ,[AuditEmailAddress].[ModifiedDate]
          ,[AuditEmailAddress].[Uuid]
        FROM
          [Audit].[AuditEmailAddress] AS [AuditEmailAddress] WITH(NOLOCK)
        WHERE
          [AuditEmailAddress].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditEmailAddress].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('EmailAddress'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
SET @EmailAddressID = FILL IN;

EXEC Person.uspDel_EmailAddressWithAudit
...
SELECT AuditEmailAddress.*
FROM   Audit.AuditEmailAddress AS AuditEmailAddress WITH(NOLOCK)
WHERE
  AuditEmailAddress.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_EmailAddressWithAudit';
GO