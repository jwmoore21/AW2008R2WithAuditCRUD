/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Person].[uspDel_PhoneNumberTypeWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Person].[uspDel_PhoneNumberTypeWithAudit]
GO
*/

CREATE PROCEDURE [Person].[uspDel_PhoneNumberTypeWithAudit]
...
================================================================================
Schema:      Person
Object:      uspDel_PhoneNumberTypeWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_PhoneNumberTypeWithAudit' );
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'PhoneNumberType'
      ...
      (
        ...
        AND [tables].[name] = 'AuditPhoneNumberType'
      ) = 1
    ) = 1
    BEGIN
      ...
      WHERE
        [PhoneNumberType].[PhoneNumberTypeID] = @_PhoneNumberTypeID
      ;
    ...
    FROM
      [Audit].[AuditPhoneNumberType] AS [AuditPhoneNumberType] WITH(NOLOCK)
    WHERE
      [AuditPhoneNumberType].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditPhoneNumberType].[PhoneNumberTypeID]
          ,[AuditPhoneNumberType].[Name]
          ,[AuditPhoneNumberType].[RowStatus]
          ,[AuditPhoneNumberType].[CreatedBy]
          ,[AuditPhoneNumberType].[ModifiedBy]
          ,[AuditPhoneNumberType].[CreatedDate]
          ,[AuditPhoneNumberType].[ModifiedDate]
          ,[AuditPhoneNumberType].[Uuid]
        FROM
          [Audit].[AuditPhoneNumberType] AS [AuditPhoneNumberType] WITH(NOLOCK)
        WHERE
          [AuditPhoneNumberType].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditPhoneNumberType].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('PhoneNumberType'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
EXEC Person.uspDel_PhoneNumberTypeWithAudit
...
SELECT AuditPhoneNumberType.*
FROM   Audit.AuditPhoneNumberType AS AuditPhoneNumberType WITH(NOLOCK)
WHERE
  AuditPhoneNumberType.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_PhoneNumberTypeWithAudit';
GO