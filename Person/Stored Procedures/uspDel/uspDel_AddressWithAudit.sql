/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Person].[uspDel_AddressWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Person].[uspDel_AddressWithAudit]
GO
*/

CREATE PROCEDURE [Person].[uspDel_AddressWithAudit]
  @AddressID INT,
...
================================================================================
Schema:      Person
Object:      uspDel_AddressWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_AddressWithAudit' );
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'Address'
    ...
    FROM
      [Audit].[AuditAddress] AS [AuditAddress] WITH(NOLOCK)
    WHERE
      [AuditAddress].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditAddress].[AddressID]
          ,[AuditAddress].[AddressLine1]
          ,[AuditAddress].[AddressLine2]
          ,[AuditAddress].[City]
          ,[AuditAddress].[StateProvinceID]
          ,[AuditAddress].[PostalCode]
          ,[AuditAddress].[SpatialLocation]
          ,[AuditAddress].[rowguid]
          ,[AuditAddress].[RowStatus]
          ,[AuditAddress].[CreatedBy]
          ,[AuditAddress].[ModifiedBy]
          ,[AuditAddress].[CreatedDate]
          ,[AuditAddress].[ModifiedDate]
          ,[AuditAddress].[Uuid]
        FROM
          [Audit].[AuditAddress] AS [AuditAddress] WITH(NOLOCK)
        WHERE
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditAddress].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('Address'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
EXEC Person.uspDel_AddressWithAudit
  @AddressID = @AddressID,
...
SELECT AuditAddress.*
FROM   Audit.AuditAddress AS AuditAddress WITH(NOLOCK)
WHERE
  AuditAddress.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_AddressWithAudit';
GO