/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Person].[uspDel_PasswordByUuid]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Person].[uspDel_PasswordByUuid]
GO
*/

CREATE PROCEDURE [Person].[uspDel_PasswordByUuid]
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_PasswordByUuid' );
...
EXEC Person.uspDel_PasswordByUuid
  @Uuid = @Uuid,
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_PasswordByUuid';
GO