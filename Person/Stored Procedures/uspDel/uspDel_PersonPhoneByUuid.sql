/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Person].[uspDel_PersonPhoneByUuid]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Person].[uspDel_PersonPhoneByUuid]
GO
*/

CREATE PROCEDURE [Person].[uspDel_PersonPhoneByUuid]
...
================================================================================
Schema:      Person
Object:      uspDel_PersonPhoneByUuid
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_PersonPhoneByUuid' );
    ...
    (
      ...
      AND [tables].[name] = 'AuditPersonPhone'
    );
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'PersonPhone'
    ...
    FROM
      [Audit].[AuditPersonPhone] AS [AuditPersonPhone] WITH(NOLOCK)
    WHERE
      [AuditPersonPhone].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditPersonPhone].[BusinessEntityID]
          ,[AuditPersonPhone].[PhoneNumber]
          ,[AuditPersonPhone].[PhoneNumberTypeID]
          ,[AuditPersonPhone].[RowStatus]
          ,[AuditPersonPhone].[CreatedBy]
          ,[AuditPersonPhone].[ModifiedBy]
          ,[AuditPersonPhone].[CreatedDate]
          ,[AuditPersonPhone].[ModifiedDate]
          ,[AuditPersonPhone].[Uuid]
        FROM
          [Audit].[AuditPersonPhone] AS [AuditPersonPhone] WITH(NOLOCK)
        WHERE
          [AuditPersonPhone].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditPersonPhone].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('PersonPhone'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
EXEC Person.uspDel_PersonPhoneByUuid
  @Uuid = @Uuid,
...
SELECT AuditPersonPhone.*
FROM   Audit.AuditPersonPhone AS AuditPersonPhone WITH(NOLOCK)
WHERE
  AuditPersonPhone.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_PersonPhoneByUuid';
GO