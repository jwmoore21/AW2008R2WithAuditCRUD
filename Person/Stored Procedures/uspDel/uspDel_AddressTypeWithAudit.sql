/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Person].[uspDel_AddressTypeWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Person].[uspDel_AddressTypeWithAudit]
GO
*/

CREATE PROCEDURE [Person].[uspDel_AddressTypeWithAudit]
  @AddressTypeID INT,
...
================================================================================
Schema:      Person
Object:      uspDel_AddressTypeWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_AddressTypeWithAudit' );
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'AddressType'
      ...
      (
        ...
        AND [tables].[name] = 'AuditAddressType'
      ) = 1
    ) = 1
    BEGIN
      ...
      WHERE
        [AddressType].[AddressTypeID] = @_AddressTypeID
      ;
    ...
    FROM
      [Audit].[AuditAddressType] AS [AuditAddressType] WITH(NOLOCK)
    WHERE
      [AuditAddressType].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditAddressType].[AddressTypeID]
          ,[AuditAddressType].[Name]
          ,[AuditAddressType].[rowguid]
          ,[AuditAddressType].[RowStatus]
          ,[AuditAddressType].[CreatedBy]
          ,[AuditAddressType].[ModifiedBy]
          ,[AuditAddressType].[CreatedDate]
          ,[AuditAddressType].[ModifiedDate]
          ,[AuditAddressType].[Uuid]
        FROM
          [Audit].[AuditAddressType] AS [AuditAddressType] WITH(NOLOCK)
        WHERE
          [AuditAddressType].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditAddressType].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('AddressType'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
EXEC Person.uspDel_AddressTypeWithAudit
...
SELECT AuditAddressType.*
FROM   Audit.AuditAddressType AS AuditAddressType WITH(NOLOCK)
WHERE
  AuditAddressType.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_AddressTypeWithAudit';
GO