/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Person].[uspDel_CountryRegionByUuid]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Person].[uspDel_CountryRegionByUuid]
GO
*/

CREATE PROCEDURE [Person].[uspDel_CountryRegionByUuid]
...
================================================================================
Schema:      Person
Object:      uspDel_CountryRegionByUuid
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_CountryRegionByUuid' );
    ...
    (
      ...
      AND [tables].[name] = 'AuditCountryRegion'
    );
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'CountryRegion'
    ...
    FROM
      [Audit].[AuditCountryRegion] AS [AuditCountryRegion] WITH(NOLOCK)
    WHERE
      [AuditCountryRegion].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditCountryRegion].[CountryRegionCode]
          ,[AuditCountryRegion].[Name]
          ,[AuditCountryRegion].[RowStatus]
          ,[AuditCountryRegion].[CreatedBy]
          ,[AuditCountryRegion].[ModifiedBy]
          ,[AuditCountryRegion].[CreatedDate]
          ,[AuditCountryRegion].[ModifiedDate]
          ,[AuditCountryRegion].[Uuid]
        FROM
          [Audit].[AuditCountryRegion] AS [AuditCountryRegion] WITH(NOLOCK)
        WHERE
          [AuditCountryRegion].[AuditBatchId] = @_AuditBatchId;
...
EXEC Person.uspDel_CountryRegionByUuid
  @Uuid = @Uuid,
...
SELECT AuditCountryRegion.*
FROM   Audit.AuditCountryRegion AS AuditCountryRegion WITH(NOLOCK)
WHERE
  AuditCountryRegion.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_CountryRegionByUuid';
GO