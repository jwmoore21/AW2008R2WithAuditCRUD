/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Person].[uspDel_BusinessEntityWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Person].[uspDel_BusinessEntityWithAudit]
GO
*/

CREATE PROCEDURE [Person].[uspDel_BusinessEntityWithAudit]
...
================================================================================
Schema:      Person
Object:      uspDel_BusinessEntityWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_BusinessEntityWithAudit' );
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'BusinessEntity'
      ...
      (
        ...
        AND [tables].[name] = 'AuditBusinessEntity'
      ) = 1
    ) = 1
    BEGIN
      ...
      WHERE
        [BusinessEntity].[BusinessEntityID] = @_BusinessEntityID
      ;
    ...
    FROM
      [Audit].[AuditBusinessEntity] AS [AuditBusinessEntity] WITH(NOLOCK)
    WHERE
      [AuditBusinessEntity].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditBusinessEntity].[BusinessEntityID]
          ,[AuditBusinessEntity].[rowguid]
          ,[AuditBusinessEntity].[RowStatus]
          ,[AuditBusinessEntity].[CreatedBy]
          ,[AuditBusinessEntity].[ModifiedBy]
          ,[AuditBusinessEntity].[CreatedDate]
          ,[AuditBusinessEntity].[ModifiedDate]
          ,[AuditBusinessEntity].[Uuid]
        FROM
          [Audit].[AuditBusinessEntity] AS [AuditBusinessEntity] WITH(NOLOCK)
        WHERE
          [AuditBusinessEntity].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditBusinessEntity].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('BusinessEntity'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
EXEC Person.uspDel_BusinessEntityWithAudit
...
SELECT AuditBusinessEntity.*
FROM   Audit.AuditBusinessEntity AS AuditBusinessEntity WITH(NOLOCK)
WHERE
  AuditBusinessEntity.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_BusinessEntityWithAudit';
GO