/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Person].[uspDel_StateProvinceWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Person].[uspDel_StateProvinceWithAudit]
GO
*/

CREATE PROCEDURE [Person].[uspDel_StateProvinceWithAudit]
  @StateProvinceID INT,
...
================================================================================
Schema:      Person
Object:      uspDel_StateProvinceWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_StateProvinceWithAudit' );

    /* ------------------------------------------------------------------------- */
    /* Table Primary Key(s) */
    DECLARE @_StateProvinceID  INT;
    SET @_StateProvinceID = @StateProvinceID;
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'StateProvince'
      ...
      (
        ...
        AND [tables].[name] = 'AuditStateProvince'
      ) = 1
    ) = 1
    BEGIN
      ...
      WHERE
        [stateprovince].[StateProvinceID] = @_StateProvinceID
      ;
    ...
    BEGIN
      ...
      BEGIN
        ...
        OUTPUT
          ...
          [DELETED].[StateProvinceCode],
          [DELETED].[CountryRegionCode],
          [DELETED].[IsOnlyStateProvinceFlag],
          [DELETED].[Name],
          [DELETED].[TerritoryID],
          [DELETED].[rowguid],
          [DELETED].[RowStatus],
    ...
    FROM
      [Audit].[AuditStateProvince] AS [AuditStateProvince] WITH(NOLOCK)
    WHERE
      [AuditStateProvince].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditStateProvince].[StateProvinceID]
          ,[AuditStateProvince].[StateProvinceCode]
          ,[AuditStateProvince].[CountryRegionCode]
          ,[AuditStateProvince].[IsOnlyStateProvinceFlag]
          ,[AuditStateProvince].[Name]
          ,[AuditStateProvince].[TerritoryID]
          ,[AuditStateProvince].[rowguid]
          ,[AuditStateProvince].[RowStatus]
          ,[AuditStateProvince].[CreatedBy]
          ,[AuditStateProvince].[ModifiedBy]
          ,[AuditStateProvince].[CreatedDate]
          ,[AuditStateProvince].[ModifiedDate]
          ,[AuditStateProvince].[Uuid]
        FROM
          [Audit].[AuditStateProvince] AS [AuditStateProvince] WITH(NOLOCK)
        WHERE
          [AuditStateProvince].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditStateProvince].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('StateProvince'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
SET @StateProvinceID = FILL IN;

EXEC Person.uspDel_StateProvinceWithAudit
...
SELECT AuditStateProvince.*
FROM   Audit.AuditStateProvince AS AuditStateProvince WITH(NOLOCK)
WHERE
  AuditStateProvince.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_StateProvinceWithAudit';
GO