/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Person].[uspDel_ContactTypeWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Person].[uspDel_ContactTypeWithAudit]
GO
*/

CREATE PROCEDURE [Person].[uspDel_ContactTypeWithAudit]
  @ContactTypeID INT,
...
================================================================================
Schema:      Person
Object:      uspDel_ContactTypeWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_ContactTypeWithAudit' );
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'ContactType'
      ...
      (
        ...
        AND [tables].[name] = 'AuditContactType'
      ) = 1
    ) = 1
    BEGIN
      ...
      WHERE
        [contacttype].[ContactTypeID] = @_ContactTypeID
      ;
    ...
    FROM
      [Audit].[AuditContactType] AS [AuditContactType] WITH(NOLOCK)
    WHERE
      [AuditContactType].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditContactType].[ContactTypeID]
          ,[AuditContactType].[Name]
          ,[AuditContactType].[RowStatus]
          ,[AuditContactType].[CreatedBy]
          ,[AuditContactType].[ModifiedBy]
          ,[AuditContactType].[CreatedDate]
          ,[AuditContactType].[ModifiedDate]
          ,[AuditContactType].[Uuid]
        FROM
          [Audit].[AuditContactType] AS [AuditContactType] WITH(NOLOCK)
        WHERE
          [AuditContactType].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditContactType].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('ContactType'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
EXEC Person.uspDel_ContactTypeWithAudit
...
SELECT AuditContactType.*
FROM   Audit.AuditContactType AS AuditContactType WITH(NOLOCK)
WHERE
  AuditContactType.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_ContactTypeWithAudit';
GO