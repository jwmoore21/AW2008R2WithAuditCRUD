/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Person].[uspDel_PasswordWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Person].[uspDel_PasswordWithAudit]
GO
*/

CREATE PROCEDURE [Person].[uspDel_PasswordWithAudit]
...
================================================================================
Schema:      Person
Object:      uspDel_PasswordWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_PasswordWithAudit' );
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'Password'
    ...
    FROM
      [Audit].[AuditPassword] AS [AuditPassword] WITH(NOLOCK)
    WHERE
      [AuditPassword].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditPassword].[BusinessEntityID]
          ,[AuditPassword].[PasswordHash]
          ,[AuditPassword].[PasswordSalt]
          ,[AuditPassword].[rowguid]
          ,[AuditPassword].[RowStatus]
          ,[AuditPassword].[CreatedBy]
          ,[AuditPassword].[ModifiedBy]
          ,[AuditPassword].[CreatedDate]
          ,[AuditPassword].[ModifiedDate]
          ,[AuditPassword].[Uuid]
        FROM
          [Audit].[AuditPassword] AS [AuditPassword] WITH(NOLOCK)
        WHERE
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditPassword].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('Password'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
EXEC Person.uspDel_PasswordWithAudit
...
SELECT AuditPassword.*
FROM   Audit.AuditPassword AS AuditPassword WITH(NOLOCK)
WHERE
  AuditPassword.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_PasswordWithAudit';
GO