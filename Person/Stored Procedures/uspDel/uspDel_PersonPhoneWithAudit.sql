/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Person].[uspDel_PersonPhoneWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Person].[uspDel_PersonPhoneWithAudit]
GO
*/
...
================================================================================
Schema:      Person
Object:      uspDel_PersonPhoneWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_PersonPhoneWithAudit' );
...
SET @PhoneNumber = FILL IN;
DECLARE @PhoneNumberTypeID INT;
SET @PhoneNumberTypeID = FILL IN;

EXEC Person.uspDel_PersonPhoneWithAudit
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_PersonPhoneWithAudit';
GO