/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Person].[uspDel_BusinessEntityContactWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Person].[uspDel_BusinessEntityContactWithAudit]
GO
*/

CREATE PROCEDURE [Person].[uspDel_BusinessEntityContactWithAudit]
...
================================================================================
Schema:      Person
Object:      uspDel_BusinessEntityContactWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_BusinessEntityContactWithAudit' );

    /* ------------------------------------------------------------------------- */
    /* Table Primary Key(s) */
    DECLARE @_BusinessEntityID  INT;
    DECLARE @_PersonID  INT;
    DECLARE @_ContactTypeID  INT;
    SET @_BusinessEntityID = @BusinessEntityID;
    SET @_PersonID = @PersonID;
    SET @_ContactTypeID = @ContactTypeID;
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'BusinessEntityContact'
      ...
      (
        ...
        AND [tables].[name] = 'AuditBusinessEntityContact'
      ) = 1
    ) = 1
    BEGIN
    ...
    FROM
      [Audit].[AuditBusinessEntityContact] AS [AuditBusinessEntityContact] WITH(NOLOCK)
    WHERE
      [AuditBusinessEntityContact].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditBusinessEntityContact].[BusinessEntityID]
          ,[AuditBusinessEntityContact].[PersonID]
          ,[AuditBusinessEntityContact].[ContactTypeID]
          ,[AuditBusinessEntityContact].[rowguid]
          ,[AuditBusinessEntityContact].[RowStatus]
          ,[AuditBusinessEntityContact].[CreatedBy]
          ,[AuditBusinessEntityContact].[ModifiedBy]
          ,[AuditBusinessEntityContact].[CreatedDate]
          ,[AuditBusinessEntityContact].[ModifiedDate]
          ,[AuditBusinessEntityContact].[Uuid]
        FROM
          [Audit].[AuditBusinessEntityContact] AS [AuditBusinessEntityContact] WITH(NOLOCK)
        WHERE
          [AuditBusinessEntityContact].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditBusinessEntityContact].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('BusinessEntityContact'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
SET @PersonID = FILL IN;
DECLARE @ContactTypeID INT;
SET @ContactTypeID = FILL IN;

EXEC Person.uspDel_BusinessEntityContactWithAudit
...
SELECT AuditBusinessEntityContact.*
FROM   Audit.AuditBusinessEntityContact AS AuditBusinessEntityContact WITH(NOLOCK)
WHERE
  AuditBusinessEntityContact.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_BusinessEntityContactWithAudit';
GO