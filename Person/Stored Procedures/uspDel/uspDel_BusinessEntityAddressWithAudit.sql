/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Person].[uspDel_BusinessEntityAddressWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Person].[uspDel_BusinessEntityAddressWithAudit]
GO
*/

CREATE PROCEDURE [Person].[uspDel_BusinessEntityAddressWithAudit]
...
================================================================================
Schema:      Person
Object:      uspDel_BusinessEntityAddressWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_BusinessEntityAddressWithAudit' );
    ...
    SET @_AddressID = @AddressID;
    SET @_AddressTypeID = @AddressTypeID;
    ...
    (
      ...
      WHERE
        [so].[type] = 'TR'
      AND USER_NAME([so2].[uid]) = 'Person'
      AND OBJECT_NAME([so].[parent_obj]) = 'BusinessEntityAddress'
      ...
      (
        ...
        AND [tables].[name] = 'AuditBusinessEntityAddress'
      ) = 1
    ) = 1
    BEGIN
      ...
      WHERE
        [businessentityaddress].[BusinessEntityID] = @_BusinessEntityID
        AND [businessentityaddress].[AddressID] = @_AddressID
        AND [businessentityaddress].[AddressTypeID] = @_AddressTypeID
      ;
    ...
    BEGIN
      ...
      BEGIN
        ...
        INTO
          [Audit].[AuditBusinessEntityAddress]
    ...
    FROM
      [Audit].[AuditBusinessEntityAddress] AS [AuditBusinessEntityAddress] WITH(NOLOCK)
    WHERE
      [AuditBusinessEntityAddress].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditBusinessEntityAddress].[BusinessEntityID]
          ,[AuditBusinessEntityAddress].[AddressID]
          ,[AuditBusinessEntityAddress].[AddressTypeID]
          ,[AuditBusinessEntityAddress].[rowguid]
          ,[AuditBusinessEntityAddress].[RowStatus]
          ,[AuditBusinessEntityAddress].[CreatedBy]
          ,[AuditBusinessEntityAddress].[ModifiedBy]
          ,[AuditBusinessEntityAddress].[CreatedDate]
          ,[AuditBusinessEntityAddress].[ModifiedDate]
          ,[AuditBusinessEntityAddress].[Uuid]
        FROM
          [Audit].[AuditBusinessEntityAddress] AS [AuditBusinessEntityAddress] WITH(NOLOCK)
        WHERE
          [AuditBusinessEntityAddress].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditBusinessEntityAddress].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('BusinessEntityAddress'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
SET @AddressID = FILL IN;
DECLARE @AddressTypeID INT;
SET @AddressTypeID = FILL IN;

EXEC Person.uspDel_BusinessEntityAddressWithAudit
...
SELECT AuditBusinessEntityAddress.*
FROM   Audit.AuditBusinessEntityAddress AS AuditBusinessEntityAddress WITH(NOLOCK)
WHERE
  AuditBusinessEntityAddress.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_BusinessEntityAddressWithAudit';
GO