/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Person].[TRG__EmailAddress__after_upd]'))
DROP TRIGGER [Person].[TRG__EmailAddress__after_upd]
GO
*/
CREATE TRIGGER [Person].[TRG__EmailAddress__after_upd] ON [Person].[EmailAddress]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Person
Object:      TRG__EmailAddress__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__EmailAddress__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditEmailAddress]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    SELECT
      ...
      ,[AuditRecordsToSave].[EmailAddressID]
    ...
    FROM
      ...
      UPDATE Person.EmailAddress
        SET
          EmailAddress.ModifiedDate = GETUTCDATE()
      FROM Person.EmailAddress AS EmailAddress
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = EmailAddress.[Uuid]
      ;
