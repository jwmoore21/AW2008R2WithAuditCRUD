/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Person].[TRG__Person__after_upd]'))
DROP TRIGGER [Person].[TRG__Person__after_upd]
GO
*/
CREATE TRIGGER [Person].[TRG__Person__after_upd] ON [Person].[Person]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Person
Object:      TRG__Person__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__Person__after_upd';
    /* ========================================================================= */
    ...
    FROM
      ...
      UPDATE Person.Person
        SET
          Person.ModifiedDate = GETUTCDATE()
      FROM Person.Person AS Person
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = Person.[Uuid]
      ;
