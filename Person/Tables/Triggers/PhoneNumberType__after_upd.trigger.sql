/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Person].[TRG__PhoneNumberType__after_upd]'))
DROP TRIGGER [Person].[TRG__PhoneNumberType__after_upd]
GO
*/
CREATE TRIGGER [Person].[TRG__PhoneNumberType__after_upd] ON [Person].[PhoneNumberType]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Person
Object:      TRG__PhoneNumberType__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__PhoneNumberType__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditPhoneNumberType]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    SELECT
      ...
      ,@_AuditBatchId
      ,@_AuditOperation
      ,[AuditRecordsToSave].[PhoneNumberTypeID]
    ...
    FROM
      ...
      UPDATE Person.PhoneNumberType
        SET
          PhoneNumberType.ModifiedDate = GETUTCDATE()
      FROM Person.PhoneNumberType AS PhoneNumberType
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = PhoneNumberType.[Uuid]
      ;
