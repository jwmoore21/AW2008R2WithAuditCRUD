/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Person].[TRG__BusinessEntity__after_upd]'))
DROP TRIGGER [Person].[TRG__BusinessEntity__after_upd]
GO
*/
CREATE TRIGGER [Person].[TRG__BusinessEntity__after_upd] ON [Person].[BusinessEntity]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Person
Object:      TRG__BusinessEntity__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__BusinessEntity__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditBusinessEntity]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    FROM
      ...
      UPDATE Person.BusinessEntity
        SET
          BusinessEntity.ModifiedDate = GETUTCDATE()
      FROM Person.BusinessEntity AS BusinessEntity
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = BusinessEntity.[Uuid]
      ;
