/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Person].[TRG__ContactType__after_upd]'))
DROP TRIGGER [Person].[TRG__ContactType__after_upd]
GO
*/
CREATE TRIGGER [Person].[TRG__ContactType__after_upd] ON [Person].[ContactType]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Person
Object:      TRG__ContactType__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__ContactType__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditContactType]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    FROM
      ...
      UPDATE Person.ContactType
        SET
          ContactType.ModifiedDate = GETUTCDATE()
      FROM Person.ContactType AS ContactType
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = ContactType.[Uuid]
      ;
