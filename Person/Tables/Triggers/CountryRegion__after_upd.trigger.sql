/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Person].[TRG__CountryRegion__after_upd]'))
DROP TRIGGER [Person].[TRG__CountryRegion__after_upd]
GO
*/
CREATE TRIGGER [Person].[TRG__CountryRegion__after_upd] ON [Person].[CountryRegion]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Person
Object:      TRG__CountryRegion__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__CountryRegion__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditCountryRegion]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    FROM
      ...
      UPDATE Person.CountryRegion
        SET
          CountryRegion.ModifiedDate = GETUTCDATE()
      FROM Person.CountryRegion AS CountryRegion
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = CountryRegion.[Uuid]
      ;
