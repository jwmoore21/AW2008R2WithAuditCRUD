/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Person].[TRG__Address__after_upd]'))
DROP TRIGGER [Person].[TRG__Address__after_upd]
GO
*/
CREATE TRIGGER [Person].[TRG__Address__after_upd] ON [Person].[Address]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Person
Object:      TRG__Address__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__Address__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditAddress]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    SELECT
      ...
      ,@_AuditBatchId
      ,@_AuditOperation
      ,[AuditRecordsToSave].[AddressID]
      ,[AuditRecordsToSave].[AddressLine1]
      ,[AuditRecordsToSave].[AddressLine2]
      ,[AuditRecordsToSave].[City]
      ,[AuditRecordsToSave].[StateProvinceID]
      ,[AuditRecordsToSave].[PostalCode]
      ,[AuditRecordsToSave].[SpatialLocation]
