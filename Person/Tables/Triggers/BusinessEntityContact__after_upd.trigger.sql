/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Person].[TRG__BusinessEntityContact__after_upd]'))
DROP TRIGGER [Person].[TRG__BusinessEntityContact__after_upd]
GO
*/
CREATE TRIGGER [Person].[TRG__BusinessEntityContact__after_upd] ON [Person].[BusinessEntityContact]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Person
Object:      TRG__BusinessEntityContact__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__BusinessEntityContact__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditBusinessEntityContact]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    SELECT
      ...
      ,[AuditRecordsToSave].[PersonID]
      ,[AuditRecordsToSave].[ContactTypeID]
