/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Person].[TRG__AddressType__after_upd]'))
DROP TRIGGER [Person].[TRG__AddressType__after_upd]
GO
*/
CREATE TRIGGER [Person].[TRG__AddressType__after_upd] ON [Person].[AddressType]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Person
Object:      TRG__AddressType__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__AddressType__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditAddressType]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    FROM
      ...
      UPDATE Person.AddressType
        SET
          AddressType.ModifiedDate = GETUTCDATE()
      FROM Person.AddressType AS AddressType
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = AddressType.[Uuid]
      ;
