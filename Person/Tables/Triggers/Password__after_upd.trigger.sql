/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Person].[TRG__Password__after_upd]'))
DROP TRIGGER [Person].[TRG__Password__after_upd]
GO
*/
CREATE TRIGGER [Person].[TRG__Password__after_upd] ON [Person].[Password]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Person
Object:      TRG__Password__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__Password__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditPassword]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    SELECT
      ...
      ,[AuditRecordsToSave].[PasswordHash]
      ,[AuditRecordsToSave].[PasswordSalt]
    ...
    FROM
      ...
      UPDATE Person.Password
        SET
          Password.ModifiedDate = GETUTCDATE()
      FROM Person.Password AS Password
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = Password.[Uuid]
      ;
