/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Production].[TRG__ProductCostHistory__after_upd]'))
DROP TRIGGER [Production].[TRG__ProductCostHistory__after_upd]
GO
*/
CREATE TRIGGER [Production].[TRG__ProductCostHistory__after_upd] ON [Production].[ProductCostHistory]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Production
Object:      TRG__ProductCostHistory__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__ProductCostHistory__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditProductCostHistory]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    FROM
      ...
      UPDATE Production.ProductCostHistory
        SET
          ProductCostHistory.ModifiedDate = GETUTCDATE()
      FROM Production.ProductCostHistory AS ProductCostHistory
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = ProductCostHistory.[Uuid]
      ;
