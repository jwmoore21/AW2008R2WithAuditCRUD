/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Production].[TRG__ScrapReason__after_upd]'))
DROP TRIGGER [Production].[TRG__ScrapReason__after_upd]
GO
*/
CREATE TRIGGER [Production].[TRG__ScrapReason__after_upd] ON [Production].[ScrapReason]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Production
Object:      TRG__ScrapReason__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__ScrapReason__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditScrapReason]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    FROM
      ...
      UPDATE Production.ScrapReason
        SET
          ScrapReason.ModifiedDate = GETUTCDATE()
      FROM Production.ScrapReason AS ScrapReason
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = ScrapReason.[Uuid]
      ;
