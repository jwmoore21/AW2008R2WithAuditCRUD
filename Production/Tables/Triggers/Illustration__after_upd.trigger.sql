/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Production].[TRG__Illustration__after_upd]'))
DROP TRIGGER [Production].[TRG__Illustration__after_upd]
GO
*/
CREATE TRIGGER [Production].[TRG__Illustration__after_upd] ON [Production].[Illustration]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Production
Object:      TRG__Illustration__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__Illustration__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditIllustration]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    FROM
      ...
      UPDATE Production.Illustration
        SET
          Illustration.ModifiedDate = GETUTCDATE()
      FROM Production.Illustration AS Illustration
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = Illustration.[Uuid]
      ;
