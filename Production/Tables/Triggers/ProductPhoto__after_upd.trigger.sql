/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Production].[TRG__ProductPhoto__after_upd]'))
DROP TRIGGER [Production].[TRG__ProductPhoto__after_upd]
GO
*/
CREATE TRIGGER [Production].[TRG__ProductPhoto__after_upd] ON [Production].[ProductPhoto]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Production
Object:      TRG__ProductPhoto__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__ProductPhoto__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditProductPhoto]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    SELECT
      ...
      ,@_AuditBatchId
      ,@_AuditOperation
      ,[AuditRecordsToSave].[ProductPhotoID]
      ,[AuditRecordsToSave].[ThumbNailPhoto]
      ,[AuditRecordsToSave].[ThumbnailPhotoFileName]
      ,[AuditRecordsToSave].[LargePhoto]
      ,[AuditRecordsToSave].[LargePhotoFileName]
    ...
    FROM
      ...
      UPDATE Production.ProductPhoto
        SET
          ProductPhoto.ModifiedDate = GETUTCDATE()
      FROM Production.ProductPhoto AS ProductPhoto
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = ProductPhoto.[Uuid]
      ;
