/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Production].[TRG__ProductInventory__after_upd]'))
DROP TRIGGER [Production].[TRG__ProductInventory__after_upd]
GO
*/
CREATE TRIGGER [Production].[TRG__ProductInventory__after_upd] ON [Production].[ProductInventory]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Production
Object:      TRG__ProductInventory__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__ProductInventory__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditProductInventory]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    SELECT
      ...
      ,[AuditRecordsToSave].[Shelf]
      ,[AuditRecordsToSave].[Bin]
    ...
    FROM
      ...
      UPDATE Production.ProductInventory
        SET
          ProductInventory.ModifiedDate = GETUTCDATE()
      FROM Production.ProductInventory AS ProductInventory
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = ProductInventory.[Uuid]
      ;
