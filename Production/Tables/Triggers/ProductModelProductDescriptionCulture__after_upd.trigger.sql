/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Production].[TRG__ProductModelProductDescriptionCulture__after_upd]'))
DROP TRIGGER [Production].[TRG__ProductModelProductDescriptionCulture__after_upd]
GO
*/
CREATE TRIGGER [Production].[TRG__ProductModelProductDescriptionCulture__after_upd] ON [Production].[ProductModelProductDescriptionCulture]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Production
Object:      TRG__ProductModelProductDescriptionCulture__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__ProductModelProductDescriptionCulture__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditProductModelProductDescriptionCulture]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    SELECT
      ...
      ,[AuditRecordsToSave].[ProductDescriptionID]
      ,[AuditRecordsToSave].[CultureID]
    ...
    FROM
      ...
      UPDATE Production.ProductModelProductDescriptionCulture
        SET
          ProductModelProductDescriptionCulture.ModifiedDate = GETUTCDATE()
      FROM Production.ProductModelProductDescriptionCulture AS ProductModelProductDescriptionCulture
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = ProductModelProductDescriptionCulture.[Uuid]
      ;
