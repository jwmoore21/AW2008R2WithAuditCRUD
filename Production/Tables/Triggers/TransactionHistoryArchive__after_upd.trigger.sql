/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Production].[TRG__TransactionHistoryArchive__after_upd]'))
DROP TRIGGER [Production].[TRG__TransactionHistoryArchive__after_upd]
GO
*/
CREATE TRIGGER [Production].[TRG__TransactionHistoryArchive__after_upd] ON [Production].[TransactionHistoryArchive]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Production
Object:      TRG__TransactionHistoryArchive__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__TransactionHistoryArchive__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditTransactionHistoryArchive]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    FROM
      ...
      UPDATE Production.TransactionHistoryArchive
        SET
          TransactionHistoryArchive.ModifiedDate = GETUTCDATE()
      FROM Production.TransactionHistoryArchive AS TransactionHistoryArchive
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = TransactionHistoryArchive.[Uuid]
      ;
