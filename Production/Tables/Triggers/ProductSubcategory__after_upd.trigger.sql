/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Production].[TRG__ProductSubcategory__after_upd]'))
DROP TRIGGER [Production].[TRG__ProductSubcategory__after_upd]
GO
*/
CREATE TRIGGER [Production].[TRG__ProductSubcategory__after_upd] ON [Production].[ProductSubcategory]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Production
Object:      TRG__ProductSubcategory__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__ProductSubcategory__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditProductSubcategory]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    SELECT
      ...
      ,[AuditRecordsToSave].[ProductCategoryID]
    ...
    FROM
      ...
      UPDATE Production.ProductSubcategory
        SET
          ProductSubcategory.ModifiedDate = GETUTCDATE()
      FROM Production.ProductSubcategory AS ProductSubcategory
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = ProductSubcategory.[Uuid]
      ;
