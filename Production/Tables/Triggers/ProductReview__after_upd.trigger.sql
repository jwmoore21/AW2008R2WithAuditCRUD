/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Production].[TRG__ProductReview__after_upd]'))
DROP TRIGGER [Production].[TRG__ProductReview__after_upd]
GO
*/
CREATE TRIGGER [Production].[TRG__ProductReview__after_upd] ON [Production].[ProductReview]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Production
Object:      TRG__ProductReview__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__ProductReview__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditProductReview]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    SELECT
      ...
      ,@_AuditBatchId
      ,@_AuditOperation
      ,[AuditRecordsToSave].[ProductReviewID]
      ,[AuditRecordsToSave].[ProductID]
      ,[AuditRecordsToSave].[ReviewerName]
      ,[AuditRecordsToSave].[ReviewDate]
      ,[AuditRecordsToSave].[EmailAddress]
      ,[AuditRecordsToSave].[Rating]
      ,[AuditRecordsToSave].[Comments]
