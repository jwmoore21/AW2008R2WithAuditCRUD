/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Production].[TRG__Document__after_upd]'))
DROP TRIGGER [Production].[TRG__Document__after_upd]
GO
*/
CREATE TRIGGER [Production].[TRG__Document__after_upd] ON [Production].[Document]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Production
Object:      TRG__Document__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__Document__after_upd';
    /* ========================================================================= */
    ...
    FROM
      ...
      UPDATE Production.Document
        SET
          Document.ModifiedDate = GETUTCDATE()
      FROM Production.Document AS Document
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = Document.[Uuid]
      ;
