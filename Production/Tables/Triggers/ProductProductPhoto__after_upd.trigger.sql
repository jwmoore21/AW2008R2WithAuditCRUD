/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Production].[TRG__ProductProductPhoto__after_upd]'))
DROP TRIGGER [Production].[TRG__ProductProductPhoto__after_upd]
GO
*/
CREATE TRIGGER [Production].[TRG__ProductProductPhoto__after_upd] ON [Production].[ProductProductPhoto]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Production
Object:      TRG__ProductProductPhoto__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__ProductProductPhoto__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditProductProductPhoto]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    FROM
      ...
      UPDATE Production.ProductProductPhoto
        SET
          ProductProductPhoto.ModifiedDate = GETUTCDATE()
      FROM Production.ProductProductPhoto AS ProductProductPhoto
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = ProductProductPhoto.[Uuid]
      ;
