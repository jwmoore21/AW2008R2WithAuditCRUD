/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Production].[TRG__BillOfMaterials__after_upd]'))
DROP TRIGGER [Production].[TRG__BillOfMaterials__after_upd]
GO
*/
CREATE TRIGGER [Production].[TRG__BillOfMaterials__after_upd] ON [Production].[BillOfMaterials]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Production
Object:      TRG__BillOfMaterials__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__BillOfMaterials__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditBillOfMaterials]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    SELECT
      ...
      ,@_AuditBatchId
      ,@_AuditOperation
      ,[AuditRecordsToSave].[BillOfMaterialsID]
      ,[AuditRecordsToSave].[ProductAssemblyID]
      ,[AuditRecordsToSave].[ComponentID]
      ,[AuditRecordsToSave].[StartDate]
      ,[AuditRecordsToSave].[EndDate]
      ,[AuditRecordsToSave].[UnitMeasureCode]
      ,[AuditRecordsToSave].[BOMLevel]
      ,[AuditRecordsToSave].[PerAssemblyQty]
    ...
    FROM
      ...
      UPDATE Production.BillOfMaterials
        SET
          BillOfMaterials.ModifiedDate = GETUTCDATE()
      FROM Production.BillOfMaterials AS BillOfMaterials
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = BillOfMaterials.[Uuid]
      ;
