/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Production].[TRG__ProductModelIllustration__after_upd]'))
DROP TRIGGER [Production].[TRG__ProductModelIllustration__after_upd]
GO
*/
CREATE TRIGGER [Production].[TRG__ProductModelIllustration__after_upd] ON [Production].[ProductModelIllustration]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Production
Object:      TRG__ProductModelIllustration__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__ProductModelIllustration__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditProductModelIllustration]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    SELECT
      ...
      ,[AuditRecordsToSave].[IllustrationID]
    ...
    FROM
      ...
      UPDATE Production.ProductModelIllustration
        SET
          ProductModelIllustration.ModifiedDate = GETUTCDATE()
      FROM Production.ProductModelIllustration AS ProductModelIllustration
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = ProductModelIllustration.[Uuid]
      ;
