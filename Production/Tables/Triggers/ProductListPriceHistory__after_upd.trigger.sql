/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Production].[TRG__ProductListPriceHistory__after_upd]'))
DROP TRIGGER [Production].[TRG__ProductListPriceHistory__after_upd]
GO
*/
CREATE TRIGGER [Production].[TRG__ProductListPriceHistory__after_upd] ON [Production].[ProductListPriceHistory]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Production
Object:      TRG__ProductListPriceHistory__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__ProductListPriceHistory__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditProductListPriceHistory]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    FROM
      ...
      UPDATE Production.ProductListPriceHistory
        SET
          ProductListPriceHistory.ModifiedDate = GETUTCDATE()
      FROM Production.ProductListPriceHistory AS ProductListPriceHistory
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = ProductListPriceHistory.[Uuid]
      ;
