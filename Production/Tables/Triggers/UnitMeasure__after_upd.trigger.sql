/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Production].[TRG__UnitMeasure__after_upd]'))
DROP TRIGGER [Production].[TRG__UnitMeasure__after_upd]
GO
*/
CREATE TRIGGER [Production].[TRG__UnitMeasure__after_upd] ON [Production].[UnitMeasure]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Production
Object:      TRG__UnitMeasure__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__UnitMeasure__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditUnitMeasure]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    FROM
      ...
      UPDATE Production.UnitMeasure
        SET
          UnitMeasure.ModifiedDate = GETUTCDATE()
      FROM Production.UnitMeasure AS UnitMeasure
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = UnitMeasure.[Uuid]
      ;
