/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Production].[TRG__Location__after_upd]'))
DROP TRIGGER [Production].[TRG__Location__after_upd]
GO
*/
CREATE TRIGGER [Production].[TRG__Location__after_upd] ON [Production].[Location]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Production
Object:      TRG__Location__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__Location__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditLocation]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    SELECT
      ...
      ,[AuditRecordsToSave].[CostRate]
      ,[AuditRecordsToSave].[Availability]
    ...
    FROM
      ...
      UPDATE Production.Location
        SET
          Location.ModifiedDate = GETUTCDATE()
      FROM Production.Location AS Location
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = Location.[Uuid]
      ;
