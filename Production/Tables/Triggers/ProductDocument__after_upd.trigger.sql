/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Production].[TRG__ProductDocument__after_upd]'))
DROP TRIGGER [Production].[TRG__ProductDocument__after_upd]
GO
*/
CREATE TRIGGER [Production].[TRG__ProductDocument__after_upd] ON [Production].[ProductDocument]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Production
Object:      TRG__ProductDocument__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__ProductDocument__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditProductDocument]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    FROM
      ...
      UPDATE Production.ProductDocument
        SET
          ProductDocument.ModifiedDate = GETUTCDATE()
      FROM Production.ProductDocument AS ProductDocument
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = ProductDocument.[Uuid]
      ;
