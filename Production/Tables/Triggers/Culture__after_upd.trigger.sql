/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Production].[TRG__Culture__after_upd]'))
DROP TRIGGER [Production].[TRG__Culture__after_upd]
GO
*/
CREATE TRIGGER [Production].[TRG__Culture__after_upd] ON [Production].[Culture]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Production
Object:      TRG__Culture__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__Culture__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditCulture]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    FROM
      ...
      UPDATE Production.Culture
        SET
          Culture.ModifiedDate = GETUTCDATE()
      FROM Production.Culture AS Culture
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = Culture.[Uuid]
      ;
