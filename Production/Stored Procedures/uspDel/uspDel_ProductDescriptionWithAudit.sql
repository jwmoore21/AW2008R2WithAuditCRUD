/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Production].[uspDel_ProductDescriptionWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Production].[uspDel_ProductDescriptionWithAudit]
GO
*/

CREATE PROCEDURE [Production].[uspDel_ProductDescriptionWithAudit]
...
================================================================================
Schema:      Production
Object:      uspDel_ProductDescriptionWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_ProductDescriptionWithAudit' );
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'ProductDescription'
      ...
      (
        ...
        AND [tables].[name] = 'AuditProductDescription'
      ) = 1
    ) = 1
    BEGIN
      ...
      WHERE
        [ProductDescription].[ProductDescriptionID] = @_ProductDescriptionID
      ;
    ...
    FROM
      [Audit].[AuditProductDescription] AS [AuditProductDescription] WITH(NOLOCK)
    WHERE
      [AuditProductDescription].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditProductDescription].[ProductDescriptionID]
          ,[AuditProductDescription].[Description]
          ,[AuditProductDescription].[rowguid]
          ,[AuditProductDescription].[RowStatus]
          ,[AuditProductDescription].[CreatedBy]
          ,[AuditProductDescription].[ModifiedBy]
          ,[AuditProductDescription].[CreatedDate]
          ,[AuditProductDescription].[ModifiedDate]
          ,[AuditProductDescription].[Uuid]
        FROM
          [Audit].[AuditProductDescription] AS [AuditProductDescription] WITH(NOLOCK)
        WHERE
          [AuditProductDescription].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditProductDescription].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('ProductDescription'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
EXEC Production.uspDel_ProductDescriptionWithAudit
...
SELECT AuditProductDescription.*
FROM   Audit.AuditProductDescription AS AuditProductDescription WITH(NOLOCK)
WHERE
  AuditProductDescription.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_ProductDescriptionWithAudit';
GO