/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Production].[uspDel_DocumentWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Production].[uspDel_DocumentWithAudit]
GO
*/

CREATE PROCEDURE [Production].[uspDel_DocumentWithAudit]
...
================================================================================
Schema:      Production
Object:      uspDel_DocumentWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_DocumentWithAudit' );
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'Document'
    ...
    BEGIN
      ...
      WHERE
        [Document].[DocumentNode] = @_DocumentNode
      ;
    ...
    FROM
      [Audit].[AuditDocument] AS [AuditDocument] WITH(NOLOCK)
    WHERE
      [AuditDocument].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditDocument].[DocumentNode]
          ,[AuditDocument].[DocumentLevel]
          ,[AuditDocument].[Title]
          ,[AuditDocument].[Owner]
          ,[AuditDocument].[FolderFlag]
          ,[AuditDocument].[FileName]
          ,[AuditDocument].[FileExtension]
          ,[AuditDocument].[Revision]
          ,[AuditDocument].[ChangeNumber]
          ,[AuditDocument].[Status]
          ,[AuditDocument].[DocumentSummary]
          ,[AuditDocument].[Document]
          ,[AuditDocument].[rowguid]
          ,[AuditDocument].[RowStatus]
          ,[AuditDocument].[CreatedBy]
          ,[AuditDocument].[ModifiedBy]
          ,[AuditDocument].[CreatedDate]
          ,[AuditDocument].[ModifiedDate]
          ,[AuditDocument].[Uuid]
        FROM
          [Audit].[AuditDocument] AS [AuditDocument] WITH(NOLOCK)
        WHERE
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditDocument].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('Document'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
EXEC Production.uspDel_DocumentWithAudit
...
FROM   Audit.AuditDocument AS AuditDocument WITH(NOLOCK)
WHERE
  AuditDocument.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_DocumentWithAudit';
GO