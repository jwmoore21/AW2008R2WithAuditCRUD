/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Production].[uspDel_UnitMeasureWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Production].[uspDel_UnitMeasureWithAudit]
GO
*/

CREATE PROCEDURE [Production].[uspDel_UnitMeasureWithAudit]
...
================================================================================
Schema:      Production
Object:      uspDel_UnitMeasureWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_UnitMeasureWithAudit' );

    /* ------------------------------------------------------------------------- */
    /* Table Primary Key(s) */
    DECLARE @_UnitMeasureCode  NCHAR;
    SET @_UnitMeasureCode = @UnitMeasureCode;
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'UnitMeasure'
      ...
      (
        ...
        AND [tables].[name] = 'AuditUnitMeasure'
      ) = 1
    ) = 1
    BEGIN
      ...
      WHERE
        [unitmeasure].[UnitMeasureCode] = @_UnitMeasureCode
      ;
    ...
    FROM
      [Audit].[AuditUnitMeasure] AS [AuditUnitMeasure] WITH(NOLOCK)
    WHERE
      [AuditUnitMeasure].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditUnitMeasure].[UnitMeasureCode]
          ,[AuditUnitMeasure].[Name]
          ,[AuditUnitMeasure].[RowStatus]
          ,[AuditUnitMeasure].[CreatedBy]
          ,[AuditUnitMeasure].[ModifiedBy]
          ,[AuditUnitMeasure].[CreatedDate]
          ,[AuditUnitMeasure].[ModifiedDate]
          ,[AuditUnitMeasure].[Uuid]
        FROM
          [Audit].[AuditUnitMeasure] AS [AuditUnitMeasure] WITH(NOLOCK)
        WHERE
          [AuditUnitMeasure].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditUnitMeasure].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('UnitMeasure'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
DECLARE @UnitMeasureCode NCHAR;
SET @UnitMeasureCode = FILL IN;

EXEC Production.uspDel_UnitMeasureWithAudit
...
SELECT AuditUnitMeasure.*
FROM   Audit.AuditUnitMeasure AS AuditUnitMeasure WITH(NOLOCK)
WHERE
  AuditUnitMeasure.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_UnitMeasureWithAudit';
GO