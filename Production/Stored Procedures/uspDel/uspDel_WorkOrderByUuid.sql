/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Production].[uspDel_WorkOrderByUuid]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Production].[uspDel_WorkOrderByUuid]
GO
*/

CREATE PROCEDURE [Production].[uspDel_WorkOrderByUuid]
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_WorkOrderByUuid' );
...
EXEC Production.uspDel_WorkOrderByUuid
  @Uuid = @Uuid,
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_WorkOrderByUuid';
GO