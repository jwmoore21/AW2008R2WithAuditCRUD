/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Production].[uspDel_WorkOrderWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Production].[uspDel_WorkOrderWithAudit]
GO
*/

CREATE PROCEDURE [Production].[uspDel_WorkOrderWithAudit]
  @WorkOrderID INT,
...
================================================================================
Schema:      Production
Object:      uspDel_WorkOrderWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_WorkOrderWithAudit' );
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'WorkOrder'
      ...
      (
        ...
        AND [tables].[name] = 'AuditWorkOrder'
      ) = 1
    ) = 1
    BEGIN
    ...
    FROM
      [Audit].[AuditWorkOrder] AS [AuditWorkOrder] WITH(NOLOCK)
    WHERE
      [AuditWorkOrder].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditWorkOrder].[WorkOrderID]
          ,[AuditWorkOrder].[ProductID]
          ,[AuditWorkOrder].[OrderQty]
          ,[AuditWorkOrder].[StockedQty]
          ,[AuditWorkOrder].[ScrappedQty]
          ,[AuditWorkOrder].[StartDate]
          ,[AuditWorkOrder].[EndDate]
          ,[AuditWorkOrder].[DueDate]
          ,[AuditWorkOrder].[ScrapReasonID]
          ,[AuditWorkOrder].[RowStatus]
          ,[AuditWorkOrder].[CreatedBy]
          ,[AuditWorkOrder].[ModifiedBy]
          ,[AuditWorkOrder].[CreatedDate]
          ,[AuditWorkOrder].[ModifiedDate]
          ,[AuditWorkOrder].[Uuid]
        FROM
          [Audit].[AuditWorkOrder] AS [AuditWorkOrder] WITH(NOLOCK)
        WHERE
...
EXEC Production.uspDel_WorkOrderWithAudit
...
SELECT AuditWorkOrder.*
FROM   Audit.AuditWorkOrder AS AuditWorkOrder WITH(NOLOCK)
WHERE
  AuditWorkOrder.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_WorkOrderWithAudit';
GO