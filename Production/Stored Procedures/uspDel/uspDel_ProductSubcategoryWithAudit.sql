/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Production].[uspDel_ProductSubcategoryWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Production].[uspDel_ProductSubcategoryWithAudit]
GO
*/

CREATE PROCEDURE [Production].[uspDel_ProductSubcategoryWithAudit]
...
================================================================================
Schema:      Production
Object:      uspDel_ProductSubcategoryWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_ProductSubcategoryWithAudit' );

    /* ------------------------------------------------------------------------- */
    /* Table Primary Key(s) */
    DECLARE @_ProductSubcategoryID  INT;
    SET @_ProductSubcategoryID = @ProductSubcategoryID;
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'ProductSubcategory'
      ...
      (
        ...
        AND [tables].[name] = 'AuditProductSubcategory'
      ) = 1
    ) = 1
    BEGIN
      ...
      WHERE
        [ProductSubcategory].[ProductSubcategoryID] = @_ProductSubcategoryID
      ;
    ...
    FROM
      [Audit].[AuditProductSubcategory] AS [AuditProductSubcategory] WITH(NOLOCK)
    WHERE
      [AuditProductSubcategory].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditProductSubcategory].[ProductSubcategoryID]
          ,[AuditProductSubcategory].[ProductCategoryID]
          ,[AuditProductSubcategory].[Name]
          ,[AuditProductSubcategory].[rowguid]
          ,[AuditProductSubcategory].[RowStatus]
          ,[AuditProductSubcategory].[CreatedBy]
          ,[AuditProductSubcategory].[ModifiedBy]
          ,[AuditProductSubcategory].[CreatedDate]
          ,[AuditProductSubcategory].[ModifiedDate]
          ,[AuditProductSubcategory].[Uuid]
        FROM
          [Audit].[AuditProductSubcategory] AS [AuditProductSubcategory] WITH(NOLOCK)
        WHERE
          [AuditProductSubcategory].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditProductSubcategory].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('ProductSubcategory'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
SET @ProductSubcategoryID = FILL IN;

EXEC Production.uspDel_ProductSubcategoryWithAudit
...
SELECT AuditProductSubcategory.*
FROM   Audit.AuditProductSubcategory AS AuditProductSubcategory WITH(NOLOCK)
WHERE
  AuditProductSubcategory.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_ProductSubcategoryWithAudit';
GO