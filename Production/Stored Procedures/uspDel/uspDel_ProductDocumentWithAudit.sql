/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Production].[uspDel_ProductDocumentWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Production].[uspDel_ProductDocumentWithAudit]
GO
*/

CREATE PROCEDURE [Production].[uspDel_ProductDocumentWithAudit]
  @ProductID INT,
...
================================================================================
Schema:      Production
Object:      uspDel_ProductDocumentWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_ProductDocumentWithAudit' );

    /* ------------------------------------------------------------------------- */
    /* Table Primary Key(s) */
    DECLARE @_ProductID  INT;
    DECLARE @_DocumentNode  HIERARCHYID;
    SET @_ProductID = @ProductID;
    SET @_DocumentNode = @DocumentNode;
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'ProductDocument'
      ...
      (
        ...
        AND [tables].[name] = 'AuditProductDocument'
      ) = 1
    ) = 1
    BEGIN
      ...
      WHERE
        [ProductDocument].[ProductID] = @_ProductID
        AND [ProductDocument].[DocumentNode] = @_DocumentNode
      ;
    ...
    FROM
      [Audit].[AuditProductDocument] AS [AuditProductDocument] WITH(NOLOCK)
    WHERE
      [AuditProductDocument].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditProductDocument].[ProductID]
          ,[AuditProductDocument].[DocumentNode]
          ,[AuditProductDocument].[RowStatus]
          ,[AuditProductDocument].[CreatedBy]
          ,[AuditProductDocument].[ModifiedBy]
          ,[AuditProductDocument].[CreatedDate]
          ,[AuditProductDocument].[ModifiedDate]
          ,[AuditProductDocument].[Uuid]
        FROM
          [Audit].[AuditProductDocument] AS [AuditProductDocument] WITH(NOLOCK)
        WHERE
          [AuditProductDocument].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditProductDocument].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('ProductDocument'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
SET @DocumentNode = FILL IN;

EXEC Production.uspDel_ProductDocumentWithAudit
  @ProductID = @ProductID,
...
SELECT AuditProductDocument.*
FROM   Audit.AuditProductDocument AS AuditProductDocument WITH(NOLOCK)
WHERE
  AuditProductDocument.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_ProductDocumentWithAudit';
GO