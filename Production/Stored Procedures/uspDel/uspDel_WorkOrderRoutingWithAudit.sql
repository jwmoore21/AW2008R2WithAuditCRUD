/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Production].[uspDel_WorkOrderRoutingWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Production].[uspDel_WorkOrderRoutingWithAudit]
GO
*/

CREATE PROCEDURE [Production].[uspDel_WorkOrderRoutingWithAudit]
  @WorkOrderID INT,
  @ProductID INT,
...
================================================================================
Schema:      Production
Object:      uspDel_WorkOrderRoutingWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_WorkOrderRoutingWithAudit' );

    /* ------------------------------------------------------------------------- */
    /* Table Primary Key(s) */
    DECLARE @_WorkOrderID  INT;
    DECLARE @_ProductID  INT;
    DECLARE @_OperationSequence  SMALLINT;
    SET @_WorkOrderID = @WorkOrderID;
    SET @_ProductID = @ProductID;
    SET @_OperationSequence = @OperationSequence;
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'WorkOrderRouting'
      ...
      (
        ...
        AND [tables].[name] = 'AuditWorkOrderRouting'
      ) = 1
    ) = 1
    BEGIN
      ...
      WHERE
        [workorderrouting].[WorkOrderID] = @_WorkOrderID
        AND [workorderrouting].[ProductID] = @_ProductID
        AND [workorderrouting].[OperationSequence] = @_OperationSequence
      ;
    ...
    FROM
      [Audit].[AuditWorkOrderRouting] AS [AuditWorkOrderRouting] WITH(NOLOCK)
    WHERE
      [AuditWorkOrderRouting].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditWorkOrderRouting].[WorkOrderID]
          ,[AuditWorkOrderRouting].[ProductID]
          ,[AuditWorkOrderRouting].[OperationSequence]
          ,[AuditWorkOrderRouting].[LocationID]
          ,[AuditWorkOrderRouting].[ScheduledStartDate]
          ,[AuditWorkOrderRouting].[ScheduledEndDate]
          ,[AuditWorkOrderRouting].[ActualStartDate]
          ,[AuditWorkOrderRouting].[ActualEndDate]
          ,[AuditWorkOrderRouting].[ActualResourceHrs]
          ,[AuditWorkOrderRouting].[PlannedCost]
          ,[AuditWorkOrderRouting].[ActualCost]
          ,[AuditWorkOrderRouting].[RowStatus]
          ,[AuditWorkOrderRouting].[CreatedBy]
          ,[AuditWorkOrderRouting].[ModifiedBy]
          ,[AuditWorkOrderRouting].[CreatedDate]
          ,[AuditWorkOrderRouting].[ModifiedDate]
          ,[AuditWorkOrderRouting].[Uuid]
        FROM
          [Audit].[AuditWorkOrderRouting] AS [AuditWorkOrderRouting] WITH(NOLOCK)
        WHERE
          [AuditWorkOrderRouting].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditWorkOrderRouting].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('WorkOrderRouting'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
DECLARE @OperationSequence SMALLINT;
SET @OperationSequence = FILL IN;

EXEC Production.uspDel_WorkOrderRoutingWithAudit
...
SELECT AuditWorkOrderRouting.*
FROM   Audit.AuditWorkOrderRouting AS AuditWorkOrderRouting WITH(NOLOCK)
WHERE
  AuditWorkOrderRouting.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_WorkOrderRoutingWithAudit';
GO