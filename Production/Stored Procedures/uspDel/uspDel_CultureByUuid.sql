/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Production].[uspDel_CultureByUuid]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Production].[uspDel_CultureByUuid]
GO
*/

CREATE PROCEDURE [Production].[uspDel_CultureByUuid]
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_CultureByUuid' );
...
EXEC Production.uspDel_CultureByUuid
  @Uuid = @Uuid,
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_CultureByUuid';
GO