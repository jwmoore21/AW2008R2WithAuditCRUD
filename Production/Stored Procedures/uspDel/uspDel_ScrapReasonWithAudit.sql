/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Production].[uspDel_ScrapReasonWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Production].[uspDel_ScrapReasonWithAudit]
GO
*/

CREATE PROCEDURE [Production].[uspDel_ScrapReasonWithAudit]
...
================================================================================
Schema:      Production
Object:      uspDel_ScrapReasonWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_ScrapReasonWithAudit' );

    /* ------------------------------------------------------------------------- */
    /* Table Primary Key(s) */
    DECLARE @_ScrapReasonID  SMALLINT;
    SET @_ScrapReasonID = @ScrapReasonID;
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'ScrapReason'
      ...
      (
        ...
        AND [tables].[name] = 'AuditScrapReason'
      ) = 1
    ) = 1
    BEGIN
      ...
      WHERE
        [ScrapReason].[ScrapReasonID] = @_ScrapReasonID
      ;
    ...
    FROM
      [Audit].[AuditScrapReason] AS [AuditScrapReason] WITH(NOLOCK)
    WHERE
      [AuditScrapReason].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditScrapReason].[ScrapReasonID]
          ,[AuditScrapReason].[Name]
          ,[AuditScrapReason].[RowStatus]
          ,[AuditScrapReason].[CreatedBy]
          ,[AuditScrapReason].[ModifiedBy]
          ,[AuditScrapReason].[CreatedDate]
          ,[AuditScrapReason].[ModifiedDate]
          ,[AuditScrapReason].[Uuid]
        FROM
          [Audit].[AuditScrapReason] AS [AuditScrapReason] WITH(NOLOCK)
        WHERE
          [AuditScrapReason].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditScrapReason].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('ScrapReason'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
DECLARE @ScrapReasonID SMALLINT;
SET @ScrapReasonID = FILL IN;

EXEC Production.uspDel_ScrapReasonWithAudit
...
SELECT AuditScrapReason.*
FROM   Audit.AuditScrapReason AS AuditScrapReason WITH(NOLOCK)
WHERE
  AuditScrapReason.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_ScrapReasonWithAudit';
GO