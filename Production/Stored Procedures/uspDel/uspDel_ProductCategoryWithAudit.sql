/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Production].[uspDel_ProductCategoryWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Production].[uspDel_ProductCategoryWithAudit]
GO
*/

CREATE PROCEDURE [Production].[uspDel_ProductCategoryWithAudit]
...
================================================================================
Schema:      Production
Object:      uspDel_ProductCategoryWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_ProductCategoryWithAudit' );

    /* ------------------------------------------------------------------------- */
    /* Table Primary Key(s) */
    DECLARE @_ProductCategoryID  INT;
    SET @_ProductCategoryID = @ProductCategoryID;
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'ProductCategory'
      ...
      (
        ...
        AND [tables].[name] = 'AuditProductCategory'
      ) = 1
    ) = 1
    BEGIN
      ...
      WHERE
        [productcategory].[ProductCategoryID] = @_ProductCategoryID
      ;
    ...
    FROM
      [Audit].[AuditProductCategory] AS [AuditProductCategory] WITH(NOLOCK)
    WHERE
      [AuditProductCategory].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditProductCategory].[ProductCategoryID]
          ,[AuditProductCategory].[Name]
          ,[AuditProductCategory].[rowguid]
          ,[AuditProductCategory].[RowStatus]
          ,[AuditProductCategory].[CreatedBy]
          ,[AuditProductCategory].[ModifiedBy]
          ,[AuditProductCategory].[CreatedDate]
          ,[AuditProductCategory].[ModifiedDate]
          ,[AuditProductCategory].[Uuid]
        FROM
          [Audit].[AuditProductCategory] AS [AuditProductCategory] WITH(NOLOCK)
        WHERE
          [AuditProductCategory].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditProductCategory].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('ProductCategory'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
SET @ProductCategoryID = FILL IN;

EXEC Production.uspDel_ProductCategoryWithAudit
...
SELECT AuditProductCategory.*
FROM   Audit.AuditProductCategory AS AuditProductCategory WITH(NOLOCK)
WHERE
  AuditProductCategory.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_ProductCategoryWithAudit';
GO