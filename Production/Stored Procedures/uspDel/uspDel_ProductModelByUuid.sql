/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Production].[uspDel_ProductModelByUuid]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Production].[uspDel_ProductModelByUuid]
GO
*/

CREATE PROCEDURE [Production].[uspDel_ProductModelByUuid]
...
================================================================================
Schema:      Production
Object:      uspDel_ProductModelByUuid
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_ProductModelByUuid' );
    ...
    (
      ...
      AND [tables].[name] = 'AuditProductModel'
    );
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'ProductModel'
    ...
    FROM
      [Audit].[AuditProductModel] AS [AuditProductModel] WITH(NOLOCK)
    WHERE
      [AuditProductModel].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditProductModel].[ProductModelID]
          ,[AuditProductModel].[Name]
          ,[AuditProductModel].[CatalogDescription]
          ,[AuditProductModel].[Instructions]
          ,[AuditProductModel].[rowguid]
          ,[AuditProductModel].[RowStatus]
          ,[AuditProductModel].[CreatedBy]
          ,[AuditProductModel].[ModifiedBy]
          ,[AuditProductModel].[CreatedDate]
          ,[AuditProductModel].[ModifiedDate]
          ,[AuditProductModel].[Uuid]
        FROM
          [Audit].[AuditProductModel] AS [AuditProductModel] WITH(NOLOCK)
        WHERE
          [AuditProductModel].[AuditBatchId] = @_AuditBatchId;
...
EXEC Production.uspDel_ProductModelByUuid
  @Uuid = @Uuid,
...
SELECT AuditProductModel.*
FROM   Audit.AuditProductModel AS AuditProductModel WITH(NOLOCK)
WHERE
  AuditProductModel.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_ProductModelByUuid';
GO