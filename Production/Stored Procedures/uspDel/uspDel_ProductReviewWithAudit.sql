/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Production].[uspDel_ProductReviewWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Production].[uspDel_ProductReviewWithAudit]
GO
*/

CREATE PROCEDURE [Production].[uspDel_ProductReviewWithAudit]
  @ProductReviewID INT,
...
================================================================================
Schema:      Production
Object:      uspDel_ProductReviewWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_ProductReviewWithAudit' );

    /* ------------------------------------------------------------------------- */
    /* Table Primary Key(s) */
    DECLARE @_ProductReviewID  INT;
    SET @_ProductReviewID = @ProductReviewID;
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'ProductReview'
      ...
      (
        ...
        AND [tables].[name] = 'AuditProductReview'
      ) = 1
    ) = 1
    BEGIN
      ...
      WHERE
        [productreview].[ProductReviewID] = @_ProductReviewID
      ;
    ...
    FROM
      [Audit].[AuditProductReview] AS [AuditProductReview] WITH(NOLOCK)
    WHERE
      [AuditProductReview].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditProductReview].[ProductReviewID]
          ,[AuditProductReview].[ProductID]
          ,[AuditProductReview].[ReviewerName]
          ,[AuditProductReview].[ReviewDate]
          ,[AuditProductReview].[EmailAddress]
          ,[AuditProductReview].[Rating]
          ,[AuditProductReview].[Comments]
          ,[AuditProductReview].[RowStatus]
          ,[AuditProductReview].[CreatedBy]
          ,[AuditProductReview].[ModifiedBy]
          ,[AuditProductReview].[CreatedDate]
          ,[AuditProductReview].[ModifiedDate]
          ,[AuditProductReview].[Uuid]
        FROM
          [Audit].[AuditProductReview] AS [AuditProductReview] WITH(NOLOCK)
        WHERE
          [AuditProductReview].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditProductReview].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('ProductReview'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
DECLARE @ProductReviewID INT;
SET @ProductReviewID = FILL IN;

EXEC Production.uspDel_ProductReviewWithAudit
...
SELECT AuditProductReview.*
FROM   Audit.AuditProductReview AS AuditProductReview WITH(NOLOCK)
WHERE
  AuditProductReview.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_ProductReviewWithAudit';
GO