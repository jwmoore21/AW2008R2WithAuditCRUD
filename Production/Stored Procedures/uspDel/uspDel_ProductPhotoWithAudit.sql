/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Production].[uspDel_ProductPhotoWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Production].[uspDel_ProductPhotoWithAudit]
GO
*/

CREATE PROCEDURE [Production].[uspDel_ProductPhotoWithAudit]
  @ProductPhotoID INT,
...
================================================================================
Schema:      Production
Object:      uspDel_ProductPhotoWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_ProductPhotoWithAudit' );
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'ProductPhoto'
      ...
      (
        ...
        AND [tables].[name] = 'AuditProductPhoto'
      ) = 1
    ) = 1
    BEGIN
      ...
      WHERE
        [ProductPhoto].[ProductPhotoID] = @_ProductPhotoID
      ;
    ...
    BEGIN
      ...
      BEGIN
        ...
        OUTPUT
          ...
          [DELETED].[ThumbNailPhoto],
          [DELETED].[ThumbnailPhotoFileName],
          [DELETED].[LargePhoto],
          [DELETED].[LargePhotoFileName],
          [DELETED].[RowStatus],
          [DELETED].[CreatedBy],
          [DELETED].[ModifiedBy],
          [DELETED].[CreatedDate],
    ...
    FROM
      [Audit].[AuditProductPhoto] AS [AuditProductPhoto] WITH(NOLOCK)
    WHERE
      [AuditProductPhoto].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditProductPhoto].[ProductPhotoID]
          ,[AuditProductPhoto].[ThumbNailPhoto]
          ,[AuditProductPhoto].[ThumbnailPhotoFileName]
          ,[AuditProductPhoto].[LargePhoto]
          ,[AuditProductPhoto].[LargePhotoFileName]
          ,[AuditProductPhoto].[RowStatus]
          ,[AuditProductPhoto].[CreatedBy]
          ,[AuditProductPhoto].[ModifiedBy]
          ,[AuditProductPhoto].[CreatedDate]
          ,[AuditProductPhoto].[ModifiedDate]
          ,[AuditProductPhoto].[Uuid]
        FROM
          [Audit].[AuditProductPhoto] AS [AuditProductPhoto] WITH(NOLOCK)
        WHERE
          [AuditProductPhoto].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditProductPhoto].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('ProductPhoto'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
EXEC Production.uspDel_ProductPhotoWithAudit
...
SELECT AuditProductPhoto.*
FROM   Audit.AuditProductPhoto AS AuditProductPhoto WITH(NOLOCK)
WHERE
  AuditProductPhoto.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_ProductPhotoWithAudit';
GO