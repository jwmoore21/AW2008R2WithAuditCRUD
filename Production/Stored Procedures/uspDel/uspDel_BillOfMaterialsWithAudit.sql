/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Production].[uspDel_BillOfMaterialsWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Production].[uspDel_BillOfMaterialsWithAudit]
GO
*/

CREATE PROCEDURE [Production].[uspDel_BillOfMaterialsWithAudit]
...
================================================================================
Schema:      Production
Object:      uspDel_BillOfMaterialsWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_BillOfMaterialsWithAudit' );

    /* ------------------------------------------------------------------------- */
    /* Table Primary Key(s) */
    DECLARE @_BillOfMaterialsID  INT;
    SET @_BillOfMaterialsID = @BillOfMaterialsID;
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'BillOfMaterials'
      ...
      (
        ...
        AND [tables].[name] = 'AuditBillOfMaterials'
      ) = 1
    ) = 1
    BEGIN
      ...
      WHERE
        [billofmaterials].[BillOfMaterialsID] = @_BillOfMaterialsID
      ;
    ...
    BEGIN
      ...
      BEGIN
        ...
        OUTPUT
          ...
          @_AuditBatchId,
          @_AuditOperation,
          [DELETED].[BillOfMaterialsID],
          [DELETED].[ProductAssemblyID],
          [DELETED].[ComponentID],
          [DELETED].[StartDate],
          [DELETED].[EndDate],
          [DELETED].[UnitMeasureCode],
          [DELETED].[BOMLevel],
          [DELETED].[PerAssemblyQty],
          [DELETED].[RowStatus],
          [DELETED].[CreatedBy],
          [DELETED].[ModifiedBy],
          [DELETED].[CreatedDate],
    ...
    FROM
      [Audit].[AuditBillOfMaterials] AS [AuditBillOfMaterials] WITH(NOLOCK)
    WHERE
      [AuditBillOfMaterials].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditBillOfMaterials].[BillOfMaterialsID]
          ,[AuditBillOfMaterials].[ProductAssemblyID]
          ,[AuditBillOfMaterials].[ComponentID]
          ,[AuditBillOfMaterials].[StartDate]
          ,[AuditBillOfMaterials].[EndDate]
          ,[AuditBillOfMaterials].[UnitMeasureCode]
          ,[AuditBillOfMaterials].[BOMLevel]
          ,[AuditBillOfMaterials].[PerAssemblyQty]
          ,[AuditBillOfMaterials].[RowStatus]
          ,[AuditBillOfMaterials].[CreatedBy]
          ,[AuditBillOfMaterials].[ModifiedBy]
          ,[AuditBillOfMaterials].[CreatedDate]
          ,[AuditBillOfMaterials].[ModifiedDate]
          ,[AuditBillOfMaterials].[Uuid]
        FROM
          [Audit].[AuditBillOfMaterials] AS [AuditBillOfMaterials] WITH(NOLOCK)
        WHERE
          [AuditBillOfMaterials].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditBillOfMaterials].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('BillOfMaterials'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
DECLARE @BillOfMaterialsID INT;
SET @BillOfMaterialsID = FILL IN;

EXEC Production.uspDel_BillOfMaterialsWithAudit
...
SELECT AuditBillOfMaterials.*
FROM   Audit.AuditBillOfMaterials AS AuditBillOfMaterials WITH(NOLOCK)
WHERE
  AuditBillOfMaterials.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_BillOfMaterialsWithAudit';
GO