/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Production].[uspDel_ProductModelIllustrationWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Production].[uspDel_ProductModelIllustrationWithAudit]
GO
*/

CREATE PROCEDURE [Production].[uspDel_ProductModelIllustrationWithAudit]
  @ProductModelID INT,
  @IllustrationID INT,
...
================================================================================
Schema:      Production
Object:      uspDel_ProductModelIllustrationWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_ProductModelIllustrationWithAudit' );

    /* ------------------------------------------------------------------------- */
    /* Table Primary Key(s) */
    DECLARE @_ProductModelID  INT;
    DECLARE @_IllustrationID  INT;
    SET @_ProductModelID = @ProductModelID;
    SET @_IllustrationID = @IllustrationID;
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'ProductModelIllustration'
      ...
      (
        ...
        AND [tables].[name] = 'AuditProductModelIllustration'
      ) = 1
    ) = 1
    BEGIN
      ...
      WHERE
        [ProductModelIllustration].[ProductModelID] = @_ProductModelID
        AND [ProductModelIllustration].[IllustrationID] = @_IllustrationID
      ;
    ...
    BEGIN
      ...
      BEGIN
        ...
        INTO
          [Audit].[AuditProductModelIllustration]
    ...
    FROM
      [Audit].[AuditProductModelIllustration] AS [AuditProductModelIllustration] WITH(NOLOCK)
    WHERE
      [AuditProductModelIllustration].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditProductModelIllustration].[ProductModelID]
          ,[AuditProductModelIllustration].[IllustrationID]
          ,[AuditProductModelIllustration].[RowStatus]
          ,[AuditProductModelIllustration].[CreatedBy]
          ,[AuditProductModelIllustration].[ModifiedBy]
          ,[AuditProductModelIllustration].[CreatedDate]
          ,[AuditProductModelIllustration].[ModifiedDate]
          ,[AuditProductModelIllustration].[Uuid]
        FROM
          [Audit].[AuditProductModelIllustration] AS [AuditProductModelIllustration] WITH(NOLOCK)
        WHERE
          [AuditProductModelIllustration].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditProductModelIllustration].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('ProductModelIllustration'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
DECLARE @IllustrationID INT;
SET @IllustrationID = FILL IN;

EXEC Production.uspDel_ProductModelIllustrationWithAudit
...
SELECT AuditProductModelIllustration.*
FROM   Audit.AuditProductModelIllustration AS AuditProductModelIllustration WITH(NOLOCK)
WHERE
  AuditProductModelIllustration.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_ProductModelIllustrationWithAudit';
GO