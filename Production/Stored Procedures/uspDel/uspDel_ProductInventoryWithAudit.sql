/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Production].[uspDel_ProductInventoryWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Production].[uspDel_ProductInventoryWithAudit]
GO
*/

CREATE PROCEDURE [Production].[uspDel_ProductInventoryWithAudit]
  @ProductID INT,
  @LocationID SMALLINT,
...
================================================================================
Schema:      Production
Object:      uspDel_ProductInventoryWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_ProductInventoryWithAudit' );

    /* ------------------------------------------------------------------------- */
    /* Table Primary Key(s) */
    DECLARE @_ProductID  INT;
    DECLARE @_LocationID  SMALLINT;
    SET @_ProductID = @ProductID;
    SET @_LocationID = @LocationID;
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'ProductInventory'
      ...
      (
        ...
        AND [tables].[name] = 'AuditProductInventory'
      ) = 1
    ) = 1
    BEGIN
    ...
    FROM
      [Audit].[AuditProductInventory] AS [AuditProductInventory] WITH(NOLOCK)
    WHERE
      [AuditProductInventory].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditProductInventory].[ProductID]
          ,[AuditProductInventory].[LocationID]
          ,[AuditProductInventory].[Shelf]
          ,[AuditProductInventory].[Bin]
          ,[AuditProductInventory].[Quantity]
          ,[AuditProductInventory].[rowguid]
          ,[AuditProductInventory].[RowStatus]
          ,[AuditProductInventory].[CreatedBy]
          ,[AuditProductInventory].[ModifiedBy]
          ,[AuditProductInventory].[CreatedDate]
          ,[AuditProductInventory].[ModifiedDate]
          ,[AuditProductInventory].[Uuid]
        FROM
          [Audit].[AuditProductInventory] AS [AuditProductInventory] WITH(NOLOCK)
        WHERE
          [AuditProductInventory].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditProductInventory].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('ProductInventory'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
SET @LocationID = FILL IN;

EXEC Production.uspDel_ProductInventoryWithAudit
  @ProductID = @ProductID,
...
SELECT AuditProductInventory.*
FROM   Audit.AuditProductInventory AS AuditProductInventory WITH(NOLOCK)
WHERE
  AuditProductInventory.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_ProductInventoryWithAudit';
GO