/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Production].[uspDel_ProductCostHistoryWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Production].[uspDel_ProductCostHistoryWithAudit]
GO
*/

CREATE PROCEDURE [Production].[uspDel_ProductCostHistoryWithAudit]
  @ProductID INT,
  @StartDate DATETIME,
...
================================================================================
Schema:      Production
Object:      uspDel_ProductCostHistoryWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_ProductCostHistoryWithAudit' );
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'ProductCostHistory'
      ...
      (
        ...
        AND [tables].[name] = 'AuditProductCostHistory'
      ) = 1
    ) = 1
    BEGIN
      ...
      WHERE
        [ProductCostHistory].[ProductID] = @_ProductID
        AND [ProductCostHistory].[StartDate] = @_StartDate
      ;
    ...
    FROM
      [Audit].[AuditProductCostHistory] AS [AuditProductCostHistory] WITH(NOLOCK)
    WHERE
      [AuditProductCostHistory].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditProductCostHistory].[ProductID]
          ,[AuditProductCostHistory].[StartDate]
          ,[AuditProductCostHistory].[EndDate]
          ,[AuditProductCostHistory].[StandardCost]
          ,[AuditProductCostHistory].[RowStatus]
          ,[AuditProductCostHistory].[CreatedBy]
          ,[AuditProductCostHistory].[ModifiedBy]
          ,[AuditProductCostHistory].[CreatedDate]
          ,[AuditProductCostHistory].[ModifiedDate]
          ,[AuditProductCostHistory].[Uuid]
        FROM
          [Audit].[AuditProductCostHistory] AS [AuditProductCostHistory] WITH(NOLOCK)
        WHERE
          [AuditProductCostHistory].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditProductCostHistory].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('ProductCostHistory'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
EXEC Production.uspDel_ProductCostHistoryWithAudit
  @ProductID = @ProductID,
  @StartDate = @StartDate,
...
SELECT AuditProductCostHistory.*
FROM   Audit.AuditProductCostHistory AS AuditProductCostHistory WITH(NOLOCK)
WHERE
  AuditProductCostHistory.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_ProductCostHistoryWithAudit';
GO