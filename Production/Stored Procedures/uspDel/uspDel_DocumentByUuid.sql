/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Production].[uspDel_DocumentByUuid]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Production].[uspDel_DocumentByUuid]
GO
*/

CREATE PROCEDURE [Production].[uspDel_DocumentByUuid]
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_DocumentByUuid' );
...
EXEC Production.uspDel_DocumentByUuid
  @Uuid = @Uuid,
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_DocumentByUuid';
GO