/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Production].[uspDel_LocationWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Production].[uspDel_LocationWithAudit]
GO
*/

CREATE PROCEDURE [Production].[uspDel_LocationWithAudit]
  @LocationID SMALLINT,
...
================================================================================
Schema:      Production
Object:      uspDel_LocationWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_LocationWithAudit' );
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'Location'
    ...
    FROM
      [Audit].[AuditLocation] AS [AuditLocation] WITH(NOLOCK)
    WHERE
      [AuditLocation].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditLocation].[LocationID]
          ,[AuditLocation].[Name]
          ,[AuditLocation].[CostRate]
          ,[AuditLocation].[Availability]
          ,[AuditLocation].[RowStatus]
          ,[AuditLocation].[CreatedBy]
          ,[AuditLocation].[ModifiedBy]
          ,[AuditLocation].[CreatedDate]
          ,[AuditLocation].[ModifiedDate]
          ,[AuditLocation].[Uuid]
        FROM
          [Audit].[AuditLocation] AS [AuditLocation] WITH(NOLOCK)
        WHERE
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditLocation].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('Location'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
EXEC Production.uspDel_LocationWithAudit
...
SELECT AuditLocation.*
FROM   Audit.AuditLocation AS AuditLocation WITH(NOLOCK)
WHERE
  AuditLocation.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_LocationWithAudit';
GO