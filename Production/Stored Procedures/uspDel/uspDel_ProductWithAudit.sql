/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Production].[uspDel_ProductWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Production].[uspDel_ProductWithAudit]
GO
*/

CREATE PROCEDURE [Production].[uspDel_ProductWithAudit]
  @ProductID INT,
...
================================================================================
Schema:      Production
Object:      uspDel_ProductWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_ProductWithAudit' );
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'Product'
    ...
    FROM
      [Audit].[AuditProduct] AS [AuditProduct] WITH(NOLOCK)
    WHERE
      [AuditProduct].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
          ...
          ,[AuditProduct].[Name]
          ,[AuditProduct].[ProductNumber]
          ,[AuditProduct].[MakeFlag]
          ,[AuditProduct].[FinishedGoodsFlag]
          ,[AuditProduct].[Color]
          ,[AuditProduct].[SafetyStockLevel]
          ,[AuditProduct].[ReorderPoint]
          ,[AuditProduct].[StandardCost]
          ,[AuditProduct].[ListPrice]
          ,[AuditProduct].[Size]
          ,[AuditProduct].[SizeUnitMeasureCode]
          ,[AuditProduct].[WeightUnitMeasureCode]
          ,[AuditProduct].[Weight]
          ,[AuditProduct].[DaysToManufacture]
          ,[AuditProduct].[ProductLine]
          ,[AuditProduct].[Class]
          ,[AuditProduct].[Style]
          ,[AuditProduct].[ProductSubcategoryID]
          ,[AuditProduct].[ProductModelID]
          ,[AuditProduct].[SellStartDate]
          ,[AuditProduct].[SellEndDate]
          ,[AuditProduct].[DiscontinuedDate]
          ,[AuditProduct].[rowguid]
          ,[AuditProduct].[RowStatus]
          ,[AuditProduct].[CreatedBy]
          ,[AuditProduct].[ModifiedBy]
          ,[AuditProduct].[CreatedDate]
          ,[AuditProduct].[ModifiedDate]
          ,[AuditProduct].[Uuid]
        FROM
          [Audit].[AuditProduct] AS [AuditProduct] WITH(NOLOCK)
        WHERE
...
EXEC Production.uspDel_ProductWithAudit
  @ProductID = @ProductID,
...
SELECT AuditProduct.*
FROM   Audit.AuditProduct AS AuditProduct WITH(NOLOCK)
WHERE
  AuditProduct.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_ProductWithAudit';
GO