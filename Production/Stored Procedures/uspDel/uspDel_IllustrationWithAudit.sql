/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Production].[uspDel_IllustrationWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Production].[uspDel_IllustrationWithAudit]
GO
*/

CREATE PROCEDURE [Production].[uspDel_IllustrationWithAudit]
  @IllustrationID INT,
...
================================================================================
Schema:      Production
Object:      uspDel_IllustrationWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_IllustrationWithAudit' );
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'Illustration'
      ...
      (
        ...
        AND [tables].[name] = 'AuditIllustration'
      ) = 1
    ) = 1
    BEGIN
      ...
      WHERE
        [illustration].[IllustrationID] = @_IllustrationID
      ;
    ...
    FROM
      [Audit].[AuditIllustration] AS [AuditIllustration] WITH(NOLOCK)
    WHERE
      [AuditIllustration].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditIllustration].[IllustrationID]
          ,[AuditIllustration].[Diagram]
          ,[AuditIllustration].[RowStatus]
          ,[AuditIllustration].[CreatedBy]
          ,[AuditIllustration].[ModifiedBy]
          ,[AuditIllustration].[CreatedDate]
          ,[AuditIllustration].[ModifiedDate]
          ,[AuditIllustration].[Uuid]
        FROM
          [Audit].[AuditIllustration] AS [AuditIllustration] WITH(NOLOCK)
        WHERE
          [AuditIllustration].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditIllustration].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('Illustration'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
EXEC Production.uspDel_IllustrationWithAudit
...
SELECT AuditIllustration.*
FROM   Audit.AuditIllustration AS AuditIllustration WITH(NOLOCK)
WHERE
  AuditIllustration.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_IllustrationWithAudit';
GO