/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Purchasing].[uspDel_PurchaseOrderDetailWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Purchasing].[uspDel_PurchaseOrderDetailWithAudit]
GO
*/

CREATE PROCEDURE [Purchasing].[uspDel_PurchaseOrderDetailWithAudit]
...
================================================================================
Schema:      Purchasing
Object:      uspDel_PurchaseOrderDetailWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_PurchaseOrderDetailWithAudit' );

    /* ------------------------------------------------------------------------- */
    /* Table Primary Key(s) */
    DECLARE @_PurchaseOrderDetailID  INT;
    DECLARE @_PurchaseOrderID  INT;
    SET @_PurchaseOrderDetailID = @PurchaseOrderDetailID;
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'PurchaseOrderDetail'
      ...
      (
        ...
        AND [tables].[name] = 'AuditPurchaseOrderDetail'
      ) = 1
    ) = 1
    BEGIN
      ...
      WHERE
        [purchaseorderdetail].[PurchaseOrderDetailID] = @_PurchaseOrderDetailID
        AND [purchaseorderdetail].[PurchaseOrderID] = @_PurchaseOrderID
      ;
    ...
    BEGIN
      ...
      BEGIN
        ...
        INTO
          [Audit].[AuditPurchaseOrderDetail]
    ...
    FROM
      [Audit].[AuditPurchaseOrderDetail] AS [AuditPurchaseOrderDetail] WITH(NOLOCK)
    WHERE
      [AuditPurchaseOrderDetail].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditPurchaseOrderDetail].[PurchaseOrderDetailID]
          ,[AuditPurchaseOrderDetail].[PurchaseOrderID]
          ,[AuditPurchaseOrderDetail].[DueDate]
          ,[AuditPurchaseOrderDetail].[OrderQty]
          ,[AuditPurchaseOrderDetail].[ProductID]
          ,[AuditPurchaseOrderDetail].[UnitPrice]
          ,[AuditPurchaseOrderDetail].[LineTotal]
          ,[AuditPurchaseOrderDetail].[ReceivedQty]
          ,[AuditPurchaseOrderDetail].[RejectedQty]
          ,[AuditPurchaseOrderDetail].[StockedQty]
          ,[AuditPurchaseOrderDetail].[SubTotal]
          ,[AuditPurchaseOrderDetail].[RowStatus]
          ,[AuditPurchaseOrderDetail].[CreatedBy]
          ,[AuditPurchaseOrderDetail].[ModifiedBy]
          ,[AuditPurchaseOrderDetail].[CreatedDate]
          ,[AuditPurchaseOrderDetail].[ModifiedDate]
          ,[AuditPurchaseOrderDetail].[Uuid]
        FROM
          [Audit].[AuditPurchaseOrderDetail] AS [AuditPurchaseOrderDetail] WITH(NOLOCK)
        WHERE
          [AuditPurchaseOrderDetail].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditPurchaseOrderDetail].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('PurchaseOrderDetail'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
SET @PurchaseOrderDetailID = FILL IN;
DECLARE @PurchaseOrderID INT;
SET @PurchaseOrderID = FILL IN;

EXEC Purchasing.uspDel_PurchaseOrderDetailWithAudit
...
SELECT AuditPurchaseOrderDetail.*
FROM   Audit.AuditPurchaseOrderDetail AS AuditPurchaseOrderDetail WITH(NOLOCK)
WHERE
  AuditPurchaseOrderDetail.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_PurchaseOrderDetailWithAudit';
GO