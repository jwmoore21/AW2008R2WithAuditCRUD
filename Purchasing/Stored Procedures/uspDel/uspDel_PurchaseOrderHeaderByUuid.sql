/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Purchasing].[uspDel_PurchaseOrderHeaderByUuid]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Purchasing].[uspDel_PurchaseOrderHeaderByUuid]
GO
*/

CREATE PROCEDURE [Purchasing].[uspDel_PurchaseOrderHeaderByUuid]
...
================================================================================
Schema:      Purchasing
Object:      uspDel_PurchaseOrderHeaderByUuid
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_PurchaseOrderHeaderByUuid' );
    ...
    (
      ...
      AND [tables].[name] = 'AuditPurchaseOrderHeader'
    );
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'PurchaseOrderHeader'
    ...
    BEGIN
      ...
      WHERE
        [PurchaseOrderHeader].[Uuid] = @_Uuid
      ;
    ...
    BEGIN
      ...
      BEGIN
        ...
        INTO
          [Audit].[AuditPurchaseOrderHeader]
    ...
    FROM
      [Audit].[AuditPurchaseOrderHeader] AS [AuditPurchaseOrderHeader] WITH(NOLOCK)
    WHERE
      [AuditPurchaseOrderHeader].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditPurchaseOrderHeader].[PurchaseOrderID]
          ,[AuditPurchaseOrderHeader].[RevisionNumber]
          ,[AuditPurchaseOrderHeader].[Status]
          ,[AuditPurchaseOrderHeader].[EmployeeID]
          ,[AuditPurchaseOrderHeader].[VendorID]
          ,[AuditPurchaseOrderHeader].[ShipMethodID]
          ,[AuditPurchaseOrderHeader].[OrderDate]
          ,[AuditPurchaseOrderHeader].[ShipDate]
          ,[AuditPurchaseOrderHeader].[SubTotal]
          ,[AuditPurchaseOrderHeader].[TaxAmt]
          ,[AuditPurchaseOrderHeader].[Freight]
          ,[AuditPurchaseOrderHeader].[TotalDue]
          ,[AuditPurchaseOrderHeader].[RowStatus]
          ,[AuditPurchaseOrderHeader].[CreatedBy]
          ,[AuditPurchaseOrderHeader].[ModifiedBy]
          ,[AuditPurchaseOrderHeader].[CreatedDate]
          ,[AuditPurchaseOrderHeader].[ModifiedDate]
          ,[AuditPurchaseOrderHeader].[Uuid]
        FROM
          [Audit].[AuditPurchaseOrderHeader] AS [AuditPurchaseOrderHeader] WITH(NOLOCK)
        WHERE
          [AuditPurchaseOrderHeader].[AuditBatchId] = @_AuditBatchId;
...
EXEC Purchasing.uspDel_PurchaseOrderHeaderByUuid
  @Uuid = @Uuid,
...
SELECT AuditPurchaseOrderHeader.*
FROM   Audit.AuditPurchaseOrderHeader AS AuditPurchaseOrderHeader WITH(NOLOCK)
WHERE
  AuditPurchaseOrderHeader.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_PurchaseOrderHeaderByUuid';
GO