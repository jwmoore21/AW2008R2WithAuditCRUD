/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Purchasing].[uspDel_ShipMethodWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Purchasing].[uspDel_ShipMethodWithAudit]
GO
*/

CREATE PROCEDURE [Purchasing].[uspDel_ShipMethodWithAudit]
  @ShipMethodID INT,
...
================================================================================
Schema:      Purchasing
Object:      uspDel_ShipMethodWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_ShipMethodWithAudit' );

    /* ------------------------------------------------------------------------- */
    /* Table Primary Key(s) */
    DECLARE @_ShipMethodID  INT;
    SET @_ShipMethodID = @ShipMethodID;
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'ShipMethod'
      ...
      (
        ...
        AND [tables].[name] = 'AuditShipMethod'
      ) = 1
    ) = 1
    BEGIN
      ...
      WHERE
        [shipmethod].[ShipMethodID] = @_ShipMethodID
      ;
    ...
    FROM
      [Audit].[AuditShipMethod] AS [AuditShipMethod] WITH(NOLOCK)
    WHERE
      [AuditShipMethod].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditShipMethod].[ShipMethodID]
          ,[AuditShipMethod].[Name]
          ,[AuditShipMethod].[ShipBase]
          ,[AuditShipMethod].[ShipRate]
          ,[AuditShipMethod].[rowguid]
          ,[AuditShipMethod].[RowStatus]
          ,[AuditShipMethod].[CreatedBy]
          ,[AuditShipMethod].[ModifiedBy]
          ,[AuditShipMethod].[CreatedDate]
          ,[AuditShipMethod].[ModifiedDate]
          ,[AuditShipMethod].[Uuid]
        FROM
          [Audit].[AuditShipMethod] AS [AuditShipMethod] WITH(NOLOCK)
        WHERE
          [AuditShipMethod].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditShipMethod].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('ShipMethod'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
SET @ShipMethodID = FILL IN;

EXEC Purchasing.uspDel_ShipMethodWithAudit
...
SELECT AuditShipMethod.*
FROM   Audit.AuditShipMethod AS AuditShipMethod WITH(NOLOCK)
WHERE
  AuditShipMethod.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_ShipMethodWithAudit';
GO