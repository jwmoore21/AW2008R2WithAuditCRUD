/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Purchasing].[uspDel_VendorByUuid]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Purchasing].[uspDel_VendorByUuid]
GO
*/

CREATE PROCEDURE [Purchasing].[uspDel_VendorByUuid]
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_VendorByUuid' );
...
EXEC Purchasing.uspDel_VendorByUuid
  @Uuid = @Uuid,
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_VendorByUuid';
GO