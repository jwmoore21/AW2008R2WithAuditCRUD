/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Purchasing].[TRG__PurchaseOrderHeader__after_upd]'))
DROP TRIGGER [Purchasing].[TRG__PurchaseOrderHeader__after_upd]
GO
*/
CREATE TRIGGER [Purchasing].[TRG__PurchaseOrderHeader__after_upd] ON [Purchasing].[PurchaseOrderHeader]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Purchasing
Object:      TRG__PurchaseOrderHeader__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__PurchaseOrderHeader__after_upd';
    /* ========================================================================= */
    ...
    FROM
      ...
      UPDATE Purchasing.PurchaseOrderHeader
        SET
          PurchaseOrderHeader.ModifiedDate = GETUTCDATE()
      FROM Purchasing.PurchaseOrderHeader AS PurchaseOrderHeader
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = PurchaseOrderHeader.[Uuid]
      ;
