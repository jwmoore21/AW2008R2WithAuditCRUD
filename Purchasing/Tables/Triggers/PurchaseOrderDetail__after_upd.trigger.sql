/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Purchasing].[TRG__PurchaseOrderDetail__after_upd]'))
DROP TRIGGER [Purchasing].[TRG__PurchaseOrderDetail__after_upd]
GO
*/
CREATE TRIGGER [Purchasing].[TRG__PurchaseOrderDetail__after_upd] ON [Purchasing].[PurchaseOrderDetail]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Purchasing
Object:      TRG__PurchaseOrderDetail__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__PurchaseOrderDetail__after_upd';
    /* ========================================================================= */
    ...
    FROM
      ...
      UPDATE Purchasing.PurchaseOrderDetail
        SET
          PurchaseOrderDetail.ModifiedDate = GETUTCDATE()
      FROM Purchasing.PurchaseOrderDetail AS PurchaseOrderDetail
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = PurchaseOrderDetail.[Uuid]
      ;
