/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[HumanResources].[uspDel_EmployeePayHistoryWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [HumanResources].[uspDel_EmployeePayHistoryWithAudit]
GO
*/

CREATE PROCEDURE [HumanResources].[uspDel_EmployeePayHistoryWithAudit]
...
Object:      uspDel_EmployeePayHistoryWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_EmployeePayHistoryWithAudit' );

    /* ------------------------------------------------------------------------- */
    /* Table Primary Key(s) */
    DECLARE @_BusinessEntityID  INT;
    DECLARE @_RateChangeDate  DATETIME;
    SET @_BusinessEntityID = @BusinessEntityID;
    SET @_RateChangeDate = @RateChangeDate;
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'EmployeePayHistory'
      ...
      (
        ...
        AND [tables].[name] = 'AuditEmployeePayHistory'
      ) = 1
    ) = 1
    BEGIN
      ...
      WHERE
        [employeepayhistory].[BusinessEntityID] = @_BusinessEntityID
        AND [employeepayhistory].[RateChangeDate] = @_RateChangeDate
      ;
    ...
    BEGIN
      ...
      BEGIN
        ...
        OUTPUT
          ...
          [DELETED].[RateChangeDate],
          [DELETED].[Rate],
          [DELETED].[PayFrequency],
          [DELETED].[RowStatus],
          [DELETED].[CreatedBy],
          [DELETED].[ModifiedBy],
          [DELETED].[CreatedDate],
    ...
    FROM
      [Audit].[AuditEmployeePayHistory] AS [AuditEmployeePayHistory] WITH(NOLOCK)
    WHERE
      [AuditEmployeePayHistory].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditEmployeePayHistory].[BusinessEntityID]
          ,[AuditEmployeePayHistory].[RateChangeDate]
          ,[AuditEmployeePayHistory].[Rate]
          ,[AuditEmployeePayHistory].[PayFrequency]
          ,[AuditEmployeePayHistory].[RowStatus]
          ,[AuditEmployeePayHistory].[CreatedBy]
          ,[AuditEmployeePayHistory].[ModifiedBy]
          ,[AuditEmployeePayHistory].[CreatedDate]
          ,[AuditEmployeePayHistory].[ModifiedDate]
          ,[AuditEmployeePayHistory].[Uuid]
        FROM
          [Audit].[AuditEmployeePayHistory] AS [AuditEmployeePayHistory] WITH(NOLOCK)
        WHERE
          [AuditEmployeePayHistory].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditEmployeePayHistory].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('EmployeePayHistory'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
SET @RateChangeDate = FILL IN;

EXEC HumanResources.uspDel_EmployeePayHistoryWithAudit
...
SELECT AuditEmployeePayHistory.*
FROM   Audit.AuditEmployeePayHistory AS AuditEmployeePayHistory WITH(NOLOCK)
WHERE
  AuditEmployeePayHistory.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_EmployeePayHistoryWithAudit';
GO