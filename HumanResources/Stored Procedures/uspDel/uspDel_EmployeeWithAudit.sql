/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[HumanResources].[uspDel_EmployeeWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [HumanResources].[uspDel_EmployeeWithAudit]
GO
*/

CREATE PROCEDURE [HumanResources].[uspDel_EmployeeWithAudit]
...
Object:      uspDel_EmployeeWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_EmployeeWithAudit' );
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'Employee'
    ...
    FROM
      [Audit].[AuditEmployee] AS [AuditEmployee] WITH(NOLOCK)
    WHERE
      [AuditEmployee].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditEmployee].[BusinessEntityID]
          ,[AuditEmployee].[NationalIDNumber]
          ,[AuditEmployee].[LoginID]
          ,[AuditEmployee].[OrganizationNode]
          ,[AuditEmployee].[OrganizationLevel]
          ,[AuditEmployee].[JobTitle]
          ,[AuditEmployee].[BirthDate]
          ,[AuditEmployee].[MaritalStatus]
          ,[AuditEmployee].[Gender]
          ,[AuditEmployee].[HireDate]
          ,[AuditEmployee].[SalariedFlag]
          ,[AuditEmployee].[VacationHours]
          ,[AuditEmployee].[SickLeaveHours]
          ,[AuditEmployee].[CurrentFlag]
          ,[AuditEmployee].[rowguid]
          ,[AuditEmployee].[RowStatus]
          ,[AuditEmployee].[CreatedBy]
          ,[AuditEmployee].[ModifiedBy]
          ,[AuditEmployee].[CreatedDate]
          ,[AuditEmployee].[ModifiedDate]
          ,[AuditEmployee].[Uuid]
        FROM
          [Audit].[AuditEmployee] AS [AuditEmployee] WITH(NOLOCK)
        WHERE
...
EXEC HumanResources.uspDel_EmployeeWithAudit
...
SELECT AuditEmployee.*
FROM   Audit.AuditEmployee AS AuditEmployee WITH(NOLOCK)
WHERE
  AuditEmployee.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_EmployeeWithAudit';
GO