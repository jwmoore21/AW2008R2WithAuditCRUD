/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[HumanResources].[uspDel_JobCandidateWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [HumanResources].[uspDel_JobCandidateWithAudit]
GO
*/

CREATE PROCEDURE [HumanResources].[uspDel_JobCandidateWithAudit]
  @JobCandidateID INT,
...
Object:      uspDel_JobCandidateWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_JobCandidateWithAudit' );

    /* ------------------------------------------------------------------------- */
    /* Table Primary Key(s) */
    DECLARE @_JobCandidateID  INT;
    SET @_JobCandidateID = @JobCandidateID;
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'JobCandidate'
      ...
      (
        ...
        AND [tables].[name] = 'AuditJobCandidate'
      ) = 1
    ) = 1
    BEGIN
      ...
      WHERE
        [jobcandidate].[JobCandidateID] = @_JobCandidateID
      ;
    ...
    FROM
      [Audit].[AuditJobCandidate] AS [AuditJobCandidate] WITH(NOLOCK)
    WHERE
      [AuditJobCandidate].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditJobCandidate].[JobCandidateID]
          ,[AuditJobCandidate].[BusinessEntityID]
          ,[AuditJobCandidate].[Resume]
          ,[AuditJobCandidate].[RowStatus]
          ,[AuditJobCandidate].[CreatedBy]
          ,[AuditJobCandidate].[ModifiedBy]
          ,[AuditJobCandidate].[CreatedDate]
          ,[AuditJobCandidate].[ModifiedDate]
          ,[AuditJobCandidate].[Uuid]
        FROM
          [Audit].[AuditJobCandidate] AS [AuditJobCandidate] WITH(NOLOCK)
        WHERE
          [AuditJobCandidate].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditJobCandidate].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('JobCandidate'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
EXEC HumanResources.uspDel_JobCandidateWithAudit
...
SELECT AuditJobCandidate.*
FROM   Audit.AuditJobCandidate AS AuditJobCandidate WITH(NOLOCK)
WHERE
  AuditJobCandidate.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_JobCandidateWithAudit';
GO