/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[HumanResources].[uspDel_ShiftWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [HumanResources].[uspDel_ShiftWithAudit]
GO
*/

CREATE PROCEDURE [HumanResources].[uspDel_ShiftWithAudit]
  @ShiftID TINYINT,
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_ShiftWithAudit' );
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'Shift'
    ...
    FROM
      [Audit].[AuditShift] AS [AuditShift] WITH(NOLOCK)
    WHERE
      [AuditShift].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditShift].[ShiftID]
          ,[AuditShift].[Name]
          ,[AuditShift].[StartTime]
          ,[AuditShift].[EndTime]
          ,[AuditShift].[RowStatus]
          ,[AuditShift].[CreatedBy]
          ,[AuditShift].[ModifiedBy]
          ,[AuditShift].[CreatedDate]
          ,[AuditShift].[ModifiedDate]
          ,[AuditShift].[Uuid]
        FROM
          [Audit].[AuditShift] AS [AuditShift] WITH(NOLOCK)
        WHERE
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditShift].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('Shift'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
EXEC HumanResources.uspDel_ShiftWithAudit
  @ShiftID = @ShiftID,
...
SELECT AuditShift.*
FROM   Audit.AuditShift AS AuditShift WITH(NOLOCK)
WHERE
  AuditShift.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_ShiftWithAudit';
GO