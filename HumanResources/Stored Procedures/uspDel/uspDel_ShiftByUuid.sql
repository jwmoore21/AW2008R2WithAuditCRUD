/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[HumanResources].[uspDel_ShiftByUuid]') AND type in (N'P', N'PC'))
DROP PROCEDURE [HumanResources].[uspDel_ShiftByUuid]
GO
*/

CREATE PROCEDURE [HumanResources].[uspDel_ShiftByUuid]
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_ShiftByUuid' );
...
EXEC HumanResources.uspDel_ShiftByUuid
  @Uuid = @Uuid,
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_ShiftByUuid';
GO