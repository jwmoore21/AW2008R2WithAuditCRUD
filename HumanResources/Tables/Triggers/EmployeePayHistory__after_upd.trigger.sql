/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[HumanResources].[TRG__EmployeePayHistory__after_upd]'))
DROP TRIGGER [HumanResources].[TRG__EmployeePayHistory__after_upd]
GO
*/
CREATE TRIGGER [HumanResources].[TRG__EmployeePayHistory__after_upd] ON [HumanResources].[EmployeePayHistory]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      HumanResources
Object:      TRG__EmployeePayHistory__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__EmployeePayHistory__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditEmployeePayHistory]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    SELECT
      ...
      ,[AuditRecordsToSave].[RateChangeDate]
      ,[AuditRecordsToSave].[Rate]
      ,[AuditRecordsToSave].[PayFrequency]
