/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[HumanResources].[TRG__Shift__after_upd]'))
DROP TRIGGER [HumanResources].[TRG__Shift__after_upd]
GO
*/
CREATE TRIGGER [HumanResources].[TRG__Shift__after_upd] ON [HumanResources].[Shift]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      HumanResources
Object:      TRG__Shift__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__Shift__after_upd';
    /* ========================================================================= */
    ...
    SELECT
      ...
      ,[AuditRecordsToSave].[StartTime]
      ,[AuditRecordsToSave].[EndTime]
    ...
    FROM
      ...
      UPDATE HumanResources.Shift
        SET
          Shift.ModifiedDate = GETUTCDATE()
      FROM HumanResources.Shift AS Shift
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = Shift.[Uuid]
      ;
