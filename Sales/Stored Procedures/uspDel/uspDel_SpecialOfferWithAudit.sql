/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Sales].[uspDel_SpecialOfferWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Sales].[uspDel_SpecialOfferWithAudit]
GO
*/

CREATE PROCEDURE [Sales].[uspDel_SpecialOfferWithAudit]
  @SpecialOfferID INT,
...
================================================================================
Schema:      Sales
Object:      uspDel_SpecialOfferWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_SpecialOfferWithAudit' );
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'SpecialOffer'
      ...
      (
        ...
        AND [tables].[name] = 'AuditSpecialOffer'
      ) = 1
    ) = 1
    BEGIN
      ...
      WHERE
        [SpecialOffer].[SpecialOfferID] = @_SpecialOfferID
      ;
    ...
    FROM
      [Audit].[AuditSpecialOffer] AS [AuditSpecialOffer] WITH(NOLOCK)
    WHERE
      [AuditSpecialOffer].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditSpecialOffer].[SpecialOfferID]
          ,[AuditSpecialOffer].[Description]
          ,[AuditSpecialOffer].[DiscountPct]
          ,[AuditSpecialOffer].[Type]
          ,[AuditSpecialOffer].[Category]
          ,[AuditSpecialOffer].[StartDate]
          ,[AuditSpecialOffer].[EndDate]
          ,[AuditSpecialOffer].[MinQty]
          ,[AuditSpecialOffer].[MaxQty]
          ,[AuditSpecialOffer].[rowguid]
          ,[AuditSpecialOffer].[RowStatus]
          ,[AuditSpecialOffer].[CreatedBy]
          ,[AuditSpecialOffer].[ModifiedBy]
          ,[AuditSpecialOffer].[CreatedDate]
          ,[AuditSpecialOffer].[ModifiedDate]
          ,[AuditSpecialOffer].[Uuid]
        FROM
          [Audit].[AuditSpecialOffer] AS [AuditSpecialOffer] WITH(NOLOCK)
        WHERE
          [AuditSpecialOffer].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditSpecialOffer].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('SpecialOffer'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
EXEC Sales.uspDel_SpecialOfferWithAudit
...
SELECT AuditSpecialOffer.*
FROM   Audit.AuditSpecialOffer AS AuditSpecialOffer WITH(NOLOCK)
WHERE
  AuditSpecialOffer.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_SpecialOfferWithAudit';
GO