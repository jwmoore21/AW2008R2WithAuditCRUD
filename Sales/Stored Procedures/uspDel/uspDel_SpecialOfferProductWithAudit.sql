/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Sales].[uspDel_SpecialOfferProductWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Sales].[uspDel_SpecialOfferProductWithAudit]
GO
*/

CREATE PROCEDURE [Sales].[uspDel_SpecialOfferProductWithAudit]
  @SpecialOfferID INT,
  @ProductID INT,
...
================================================================================
Schema:      Sales
Object:      uspDel_SpecialOfferProductWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_SpecialOfferProductWithAudit' );

    /* ------------------------------------------------------------------------- */
    /* Table Primary Key(s) */
    DECLARE @_SpecialOfferID  INT;
    DECLARE @_ProductID  INT;
    SET @_SpecialOfferID = @SpecialOfferID;
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'SpecialOfferProduct'
      ...
      (
        ...
        AND [tables].[name] = 'AuditSpecialOfferProduct'
      ) = 1
    ) = 1
    BEGIN
      ...
      WHERE
        [specialofferproduct].[SpecialOfferID] = @_SpecialOfferID
        AND [specialofferproduct].[ProductID] = @_ProductID
      ;
    ...
    BEGIN
      ...
      BEGIN
        ...
        INTO
          [Audit].[AuditSpecialOfferProduct]
    ...
    FROM
      [Audit].[AuditSpecialOfferProduct] AS [AuditSpecialOfferProduct] WITH(NOLOCK)
    WHERE
      [AuditSpecialOfferProduct].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditSpecialOfferProduct].[SpecialOfferID]
          ,[AuditSpecialOfferProduct].[ProductID]
          ,[AuditSpecialOfferProduct].[rowguid]
          ,[AuditSpecialOfferProduct].[RowStatus]
          ,[AuditSpecialOfferProduct].[CreatedBy]
          ,[AuditSpecialOfferProduct].[ModifiedBy]
          ,[AuditSpecialOfferProduct].[CreatedDate]
          ,[AuditSpecialOfferProduct].[ModifiedDate]
          ,[AuditSpecialOfferProduct].[Uuid]
        FROM
          [Audit].[AuditSpecialOfferProduct] AS [AuditSpecialOfferProduct] WITH(NOLOCK)
        WHERE
          [AuditSpecialOfferProduct].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditSpecialOfferProduct].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('SpecialOfferProduct'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
SET @SpecialOfferID = FILL IN;
DECLARE @ProductID INT;
SET @ProductID = FILL IN;

EXEC Sales.uspDel_SpecialOfferProductWithAudit
...
SELECT AuditSpecialOfferProduct.*
FROM   Audit.AuditSpecialOfferProduct AS AuditSpecialOfferProduct WITH(NOLOCK)
WHERE
  AuditSpecialOfferProduct.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_SpecialOfferProductWithAudit';
GO