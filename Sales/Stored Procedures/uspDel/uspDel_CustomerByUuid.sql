/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Sales].[uspDel_CustomerByUuid]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Sales].[uspDel_CustomerByUuid]
GO
*/

CREATE PROCEDURE [Sales].[uspDel_CustomerByUuid]
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_CustomerByUuid' );
...
EXEC Sales.uspDel_CustomerByUuid
  @Uuid = @Uuid,
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_CustomerByUuid';
GO