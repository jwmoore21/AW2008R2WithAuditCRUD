/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Sales].[uspDel_CreditCardWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Sales].[uspDel_CreditCardWithAudit]
GO
*/

CREATE PROCEDURE [Sales].[uspDel_CreditCardWithAudit]
  @CreditCardID INT,
...
================================================================================
Schema:      Sales
Object:      uspDel_CreditCardWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_CreditCardWithAudit' );
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'CreditCard'
      ...
      (
        ...
        AND [tables].[name] = 'AuditCreditCard'
      ) = 1
    ) = 1
    BEGIN
      ...
      WHERE
        [creditcard].[CreditCardID] = @_CreditCardID
      ;
    ...
    FROM
      [Audit].[AuditCreditCard] AS [AuditCreditCard] WITH(NOLOCK)
    WHERE
      [AuditCreditCard].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditCreditCard].[CreditCardID]
          ,[AuditCreditCard].[CardType]
          ,[AuditCreditCard].[CardNumber]
          ,[AuditCreditCard].[ExpMonth]
          ,[AuditCreditCard].[ExpYear]
          ,[AuditCreditCard].[RowStatus]
          ,[AuditCreditCard].[CreatedBy]
          ,[AuditCreditCard].[ModifiedBy]
          ,[AuditCreditCard].[CreatedDate]
          ,[AuditCreditCard].[ModifiedDate]
          ,[AuditCreditCard].[Uuid]
        FROM
          [Audit].[AuditCreditCard] AS [AuditCreditCard] WITH(NOLOCK)
        WHERE
          [AuditCreditCard].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditCreditCard].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('CreditCard'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
EXEC Sales.uspDel_CreditCardWithAudit
...
SELECT AuditCreditCard.*
FROM   Audit.AuditCreditCard AS AuditCreditCard WITH(NOLOCK)
WHERE
  AuditCreditCard.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_CreditCardWithAudit';
GO