/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Sales].[uspDel_SalesReasonWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Sales].[uspDel_SalesReasonWithAudit]
GO
*/

CREATE PROCEDURE [Sales].[uspDel_SalesReasonWithAudit]
  @SalesReasonID INT,
...
================================================================================
Schema:      Sales
Object:      uspDel_SalesReasonWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_SalesReasonWithAudit' );
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'SalesReason'
      ...
      (
        ...
        AND [tables].[name] = 'AuditSalesReason'
      ) = 1
    ) = 1
    BEGIN
      ...
      WHERE
        [SalesReason].[SalesReasonID] = @_SalesReasonID
      ;
    ...
    FROM
      [Audit].[AuditSalesReason] AS [AuditSalesReason] WITH(NOLOCK)
    WHERE
      [AuditSalesReason].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditSalesReason].[SalesReasonID]
          ,[AuditSalesReason].[Name]
          ,[AuditSalesReason].[ReasonType]
          ,[AuditSalesReason].[RowStatus]
          ,[AuditSalesReason].[CreatedBy]
          ,[AuditSalesReason].[ModifiedBy]
          ,[AuditSalesReason].[CreatedDate]
          ,[AuditSalesReason].[ModifiedDate]
          ,[AuditSalesReason].[Uuid]
        FROM
          [Audit].[AuditSalesReason] AS [AuditSalesReason] WITH(NOLOCK)
        WHERE
          [AuditSalesReason].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditSalesReason].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('SalesReason'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
EXEC Sales.uspDel_SalesReasonWithAudit
...
SELECT AuditSalesReason.*
FROM   Audit.AuditSalesReason AS AuditSalesReason WITH(NOLOCK)
WHERE
  AuditSalesReason.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_SalesReasonWithAudit';
GO