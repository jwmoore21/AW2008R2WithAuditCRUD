/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Sales].[uspDel_CurrencyByUuid]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Sales].[uspDel_CurrencyByUuid]
GO
*/

CREATE PROCEDURE [Sales].[uspDel_CurrencyByUuid]
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_CurrencyByUuid' );
...
EXEC Sales.uspDel_CurrencyByUuid
  @Uuid = @Uuid,
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_CurrencyByUuid';
GO