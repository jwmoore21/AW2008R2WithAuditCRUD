/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Sales].[uspDel_ShoppingCartItemWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Sales].[uspDel_ShoppingCartItemWithAudit]
GO
*/

CREATE PROCEDURE [Sales].[uspDel_ShoppingCartItemWithAudit]
...
================================================================================
Schema:      Sales
Object:      uspDel_ShoppingCartItemWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_ShoppingCartItemWithAudit' );

    /* ------------------------------------------------------------------------- */
    /* Table Primary Key(s) */
    DECLARE @_ShoppingCartItemID  INT;
    SET @_ShoppingCartItemID = @ShoppingCartItemID;
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'ShoppingCartItem'
      ...
      (
        ...
        AND [tables].[name] = 'AuditShoppingCartItem'
      ) = 1
    ) = 1
    BEGIN
      ...
      WHERE
        [shoppingcartitem].[ShoppingCartItemID] = @_ShoppingCartItemID
      ;
    ...
    BEGIN
      ...
      BEGIN
        ...
        OUTPUT
          ...
          @_AuditBatchId,
          @_AuditOperation,
          [DELETED].[ShoppingCartItemID],
          [DELETED].[ShoppingCartID],
          [DELETED].[Quantity],
          [DELETED].[ProductID],
          [DELETED].[DateCreated],
          [DELETED].[RowStatus],
    ...
    FROM
      [Audit].[AuditShoppingCartItem] AS [AuditShoppingCartItem] WITH(NOLOCK)
    WHERE
      [AuditShoppingCartItem].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditShoppingCartItem].[ShoppingCartItemID]
          ,[AuditShoppingCartItem].[ShoppingCartID]
          ,[AuditShoppingCartItem].[Quantity]
          ,[AuditShoppingCartItem].[ProductID]
          ,[AuditShoppingCartItem].[DateCreated]
          ,[AuditShoppingCartItem].[RowStatus]
          ,[AuditShoppingCartItem].[CreatedBy]
          ,[AuditShoppingCartItem].[ModifiedBy]
          ,[AuditShoppingCartItem].[CreatedDate]
          ,[AuditShoppingCartItem].[ModifiedDate]
          ,[AuditShoppingCartItem].[Uuid]
        FROM
          [Audit].[AuditShoppingCartItem] AS [AuditShoppingCartItem] WITH(NOLOCK)
        WHERE
          [AuditShoppingCartItem].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditShoppingCartItem].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('ShoppingCartItem'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
SET @ShoppingCartItemID = FILL IN;

EXEC Sales.uspDel_ShoppingCartItemWithAudit
...
SELECT AuditShoppingCartItem.*
FROM   Audit.AuditShoppingCartItem AS AuditShoppingCartItem WITH(NOLOCK)
WHERE
  AuditShoppingCartItem.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_ShoppingCartItemWithAudit';
GO