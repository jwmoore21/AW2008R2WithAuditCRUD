/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Sales].[uspDel_CurrencyWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Sales].[uspDel_CurrencyWithAudit]
GO
*/

CREATE PROCEDURE [Sales].[uspDel_CurrencyWithAudit]
...
================================================================================
Schema:      Sales
Object:      uspDel_CurrencyWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_CurrencyWithAudit' );
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'Currency'
    ...
    BEGIN
      ...
      WHERE
        [Currency].[CurrencyCode] = @_CurrencyCode
      ;
    ...
    FROM
      [Audit].[AuditCurrency] AS [AuditCurrency] WITH(NOLOCK)
    WHERE
      [AuditCurrency].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditCurrency].[CurrencyCode]
          ,[AuditCurrency].[Name]
          ,[AuditCurrency].[RowStatus]
          ,[AuditCurrency].[CreatedBy]
          ,[AuditCurrency].[ModifiedBy]
          ,[AuditCurrency].[CreatedDate]
          ,[AuditCurrency].[ModifiedDate]
          ,[AuditCurrency].[Uuid]
        FROM
          [Audit].[AuditCurrency] AS [AuditCurrency] WITH(NOLOCK)
        WHERE
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditCurrency].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('Currency'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
EXEC Sales.uspDel_CurrencyWithAudit
...
SELECT AuditCurrency.*
FROM   Audit.AuditCurrency AS AuditCurrency WITH(NOLOCK)
WHERE
  AuditCurrency.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_CurrencyWithAudit';
GO