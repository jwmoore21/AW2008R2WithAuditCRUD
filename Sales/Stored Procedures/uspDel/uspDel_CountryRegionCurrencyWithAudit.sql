/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Sales].[uspDel_CountryRegionCurrencyWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Sales].[uspDel_CountryRegionCurrencyWithAudit]
GO
*/

CREATE PROCEDURE [Sales].[uspDel_CountryRegionCurrencyWithAudit]
...
================================================================================
Schema:      Sales
Object:      uspDel_CountryRegionCurrencyWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_CountryRegionCurrencyWithAudit' );

    /* ------------------------------------------------------------------------- */
    /* Table Primary Key(s) */
    DECLARE @_CountryRegionCode  NVARCHAR;
    DECLARE @_CurrencyCode  NCHAR;
    SET @_CountryRegionCode = @CountryRegionCode;
    SET @_CurrencyCode = @CurrencyCode;
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'CountryRegionCurrency'
      ...
      (
        ...
        AND [tables].[name] = 'AuditCountryRegionCurrency'
      ) = 1
    ) = 1
    BEGIN
      ...
      WHERE
        [CountryRegionCurrency].[CountryRegionCode] = @_CountryRegionCode
        AND [CountryRegionCurrency].[CurrencyCode] = @_CurrencyCode
      ;
    ...
    BEGIN
      ...
      BEGIN
        ...
        INTO
          [Audit].[AuditCountryRegionCurrency]
    ...
    FROM
      [Audit].[AuditCountryRegionCurrency] AS [AuditCountryRegionCurrency] WITH(NOLOCK)
    WHERE
      [AuditCountryRegionCurrency].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditCountryRegionCurrency].[CountryRegionCode]
          ,[AuditCountryRegionCurrency].[CurrencyCode]
          ,[AuditCountryRegionCurrency].[RowStatus]
          ,[AuditCountryRegionCurrency].[CreatedBy]
          ,[AuditCountryRegionCurrency].[ModifiedBy]
          ,[AuditCountryRegionCurrency].[CreatedDate]
          ,[AuditCountryRegionCurrency].[ModifiedDate]
          ,[AuditCountryRegionCurrency].[Uuid]
        FROM
          [Audit].[AuditCountryRegionCurrency] AS [AuditCountryRegionCurrency] WITH(NOLOCK)
        WHERE
          [AuditCountryRegionCurrency].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditCountryRegionCurrency].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('CountryRegionCurrency'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
DECLARE @CurrencyCode NCHAR;
SET @CurrencyCode = FILL IN;

EXEC Sales.uspDel_CountryRegionCurrencyWithAudit
...
SELECT AuditCountryRegionCurrency.*
FROM   Audit.AuditCountryRegionCurrency AS AuditCountryRegionCurrency WITH(NOLOCK)
WHERE
  AuditCountryRegionCurrency.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_CountryRegionCurrencyWithAudit';
GO