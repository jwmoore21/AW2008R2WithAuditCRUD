/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Sales].[uspDel_PersonCreditCardByUuid]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Sales].[uspDel_PersonCreditCardByUuid]
GO
*/

CREATE PROCEDURE [Sales].[uspDel_PersonCreditCardByUuid]
...
================================================================================
Schema:      Sales
Object:      uspDel_PersonCreditCardByUuid
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_PersonCreditCardByUuid' );
    ...
    (
      ...
      AND [tables].[name] = 'AuditPersonCreditCard'
    );
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'PersonCreditCard'
    ...
    FROM
      [Audit].[AuditPersonCreditCard] AS [AuditPersonCreditCard] WITH(NOLOCK)
    WHERE
      [AuditPersonCreditCard].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditPersonCreditCard].[BusinessEntityID]
          ,[AuditPersonCreditCard].[CreditCardID]
          ,[AuditPersonCreditCard].[RowStatus]
          ,[AuditPersonCreditCard].[CreatedBy]
          ,[AuditPersonCreditCard].[ModifiedBy]
          ,[AuditPersonCreditCard].[CreatedDate]
          ,[AuditPersonCreditCard].[ModifiedDate]
          ,[AuditPersonCreditCard].[Uuid]
        FROM
          [Audit].[AuditPersonCreditCard] AS [AuditPersonCreditCard] WITH(NOLOCK)
        WHERE
          [AuditPersonCreditCard].[AuditBatchId] = @_AuditBatchId;
...
EXEC Sales.uspDel_PersonCreditCardByUuid
  @Uuid = @Uuid,
...
SELECT AuditPersonCreditCard.*
FROM   Audit.AuditPersonCreditCard AS AuditPersonCreditCard WITH(NOLOCK)
WHERE
  AuditPersonCreditCard.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_PersonCreditCardByUuid';
GO