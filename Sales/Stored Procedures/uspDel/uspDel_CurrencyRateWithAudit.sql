/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Sales].[uspDel_CurrencyRateWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Sales].[uspDel_CurrencyRateWithAudit]
GO
*/

CREATE PROCEDURE [Sales].[uspDel_CurrencyRateWithAudit]
  @CurrencyRateID INT,
...
================================================================================
Schema:      Sales
Object:      uspDel_CurrencyRateWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_CurrencyRateWithAudit' );

    /* ------------------------------------------------------------------------- */
    /* Table Primary Key(s) */
    DECLARE @_CurrencyRateID  INT;
    SET @_CurrencyRateID = @CurrencyRateID;
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'CurrencyRate'
      ...
      (
        ...
        AND [tables].[name] = 'AuditCurrencyRate'
      ) = 1
    ) = 1
    BEGIN
      ...
      WHERE
        [currencyrate].[CurrencyRateID] = @_CurrencyRateID
      ;
    ...
    BEGIN
      ...
      BEGIN
        ...
        OUTPUT
          ...
          [DELETED].[CurrencyRateDate],
          [DELETED].[FromCurrencyCode],
          [DELETED].[ToCurrencyCode],
          [DELETED].[AverageRate],
          [DELETED].[EndOfDayRate],
          [DELETED].[RowStatus],
          [DELETED].[CreatedBy],
          [DELETED].[ModifiedBy],
          [DELETED].[CreatedDate],
    ...
    FROM
      [Audit].[AuditCurrencyRate] AS [AuditCurrencyRate] WITH(NOLOCK)
    WHERE
      [AuditCurrencyRate].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
           [AuditCurrencyRate].[CurrencyRateID]
          ,[AuditCurrencyRate].[CurrencyRateDate]
          ,[AuditCurrencyRate].[FromCurrencyCode]
          ,[AuditCurrencyRate].[ToCurrencyCode]
          ,[AuditCurrencyRate].[AverageRate]
          ,[AuditCurrencyRate].[EndOfDayRate]
          ,[AuditCurrencyRate].[RowStatus]
          ,[AuditCurrencyRate].[CreatedBy]
          ,[AuditCurrencyRate].[ModifiedBy]
          ,[AuditCurrencyRate].[CreatedDate]
          ,[AuditCurrencyRate].[ModifiedDate]
          ,[AuditCurrencyRate].[Uuid]
        FROM
          [Audit].[AuditCurrencyRate] AS [AuditCurrencyRate] WITH(NOLOCK)
        WHERE
          [AuditCurrencyRate].[AuditBatchId] = @_AuditBatchId;
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditCurrencyRate].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('CurrencyRate'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
SET @CurrencyRateID = FILL IN;

EXEC Sales.uspDel_CurrencyRateWithAudit
...
SELECT AuditCurrencyRate.*
FROM   Audit.AuditCurrencyRate AS AuditCurrencyRate WITH(NOLOCK)
WHERE
  AuditCurrencyRate.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_CurrencyRateWithAudit';
GO