/*
IF EXISTS (SELECT 1 FROM [sys].[objects] WHERE object_id = OBJECT_ID(N'[Sales].[uspDel_CustomerWithAudit]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Sales].[uspDel_CustomerWithAudit]
GO
*/

CREATE PROCEDURE [Sales].[uspDel_CustomerWithAudit]
  @CustomerID INT,
...
================================================================================
Schema:      Sales
Object:      uspDel_CustomerWithAudit
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = COALESCE( @AuditOperation, 'Delete proc ran: uspDel_CustomerWithAudit' );
    ...
    (
      ...
      AND OBJECT_NAME([so].[parent_obj]) = 'Customer'
    ...
    FROM
      [Audit].[AuditCustomer] AS [AuditCustomer] WITH(NOLOCK)
    WHERE
      [AuditCustomer].[AuditBatchId] = @_AuditBatchId;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SELECT
          ...
          ,[AuditCustomer].[PersonID]
          ,[AuditCustomer].[StoreID]
          ,[AuditCustomer].[TerritoryID]
          ,[AuditCustomer].[AccountNumber]
          ,[AuditCustomer].[rowguid]
          ,[AuditCustomer].[RowStatus]
          ,[AuditCustomer].[CreatedBy]
          ,[AuditCustomer].[ModifiedBy]
          ,[AuditCustomer].[CreatedDate]
          ,[AuditCustomer].[ModifiedDate]
          ,[AuditCustomer].[Uuid]
        FROM
          [Audit].[AuditCustomer] AS [AuditCustomer] WITH(NOLOCK)
        WHERE
        ...
        BEGIN TRY
          SELECT
            ...
            (
              ...
              WHERE
                [AuditCustomer].[AuditBatchId] = @_AuditBatchId
              FOR XML PATH ('Customer'), ROOT ('DeletedRecords'), ELEMENTS XSINIL
            );
...
SET @CustomerID = FILL IN;

EXEC Sales.uspDel_CustomerWithAudit
...
SELECT AuditCustomer.*
FROM   Audit.AuditCustomer AS AuditCustomer WITH(NOLOCK)
WHERE
  AuditCustomer.AuditBatchId = @AuditBatchId
;
...
EXECUTE sp_addextendedproperty
  ...
  @level1name = N'uspDel_CustomerWithAudit';
GO