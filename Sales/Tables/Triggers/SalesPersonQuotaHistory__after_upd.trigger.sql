/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Sales].[TRG__SalesPersonQuotaHistory__after_upd]'))
DROP TRIGGER [Sales].[TRG__SalesPersonQuotaHistory__after_upd]
GO
*/
CREATE TRIGGER [Sales].[TRG__SalesPersonQuotaHistory__after_upd] ON [Sales].[SalesPersonQuotaHistory]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Sales
Object:      TRG__SalesPersonQuotaHistory__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__SalesPersonQuotaHistory__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditSalesPersonQuotaHistory]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    SELECT
      ...
      ,[AuditRecordsToSave].[QuotaDate]
      ,[AuditRecordsToSave].[SalesQuota]
    ...
    FROM
      ...
      UPDATE Sales.SalesPersonQuotaHistory
        SET
          SalesPersonQuotaHistory.ModifiedDate = GETUTCDATE()
      FROM Sales.SalesPersonQuotaHistory AS SalesPersonQuotaHistory
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = SalesPersonQuotaHistory.[Uuid]
      ;
