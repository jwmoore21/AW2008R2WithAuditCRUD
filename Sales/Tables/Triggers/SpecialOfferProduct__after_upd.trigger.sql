/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Sales].[TRG__SpecialOfferProduct__after_upd]'))
DROP TRIGGER [Sales].[TRG__SpecialOfferProduct__after_upd]
GO
*/
CREATE TRIGGER [Sales].[TRG__SpecialOfferProduct__after_upd] ON [Sales].[SpecialOfferProduct]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Sales
Object:      TRG__SpecialOfferProduct__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__SpecialOfferProduct__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditSpecialOfferProduct]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    FROM
      ...
      UPDATE Sales.SpecialOfferProduct
        SET
          SpecialOfferProduct.ModifiedDate = GETUTCDATE()
      FROM Sales.SpecialOfferProduct AS SpecialOfferProduct
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = SpecialOfferProduct.[Uuid]
      ;
