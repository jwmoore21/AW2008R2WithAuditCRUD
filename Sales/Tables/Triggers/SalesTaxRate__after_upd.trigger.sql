/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Sales].[TRG__SalesTaxRate__after_upd]'))
DROP TRIGGER [Sales].[TRG__SalesTaxRate__after_upd]
GO
*/
CREATE TRIGGER [Sales].[TRG__SalesTaxRate__after_upd] ON [Sales].[SalesTaxRate]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Sales
Object:      TRG__SalesTaxRate__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__SalesTaxRate__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditSalesTaxRate]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    SELECT
      ...
      ,@_AuditBatchId
      ,@_AuditOperation
      ,[AuditRecordsToSave].[SalesTaxRateID]
      ,[AuditRecordsToSave].[StateProvinceID]
      ,[AuditRecordsToSave].[TaxType]
      ,[AuditRecordsToSave].[TaxRate]
    ...
    FROM
      ...
      UPDATE Sales.SalesTaxRate
        SET
          SalesTaxRate.ModifiedDate = GETUTCDATE()
      FROM Sales.SalesTaxRate AS SalesTaxRate
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = SalesTaxRate.[Uuid]
      ;
