/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Sales].[TRG__SalesOrderHeaderSalesReason__after_upd]'))
DROP TRIGGER [Sales].[TRG__SalesOrderHeaderSalesReason__after_upd]
GO
*/
CREATE TRIGGER [Sales].[TRG__SalesOrderHeaderSalesReason__after_upd] ON [Sales].[SalesOrderHeaderSalesReason]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Sales
Object:      TRG__SalesOrderHeaderSalesReason__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__SalesOrderHeaderSalesReason__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditSalesOrderHeaderSalesReason]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    SELECT
      ...
      ,[AuditRecordsToSave].[SalesReasonID]
    ...
    FROM
      ...
      UPDATE Sales.SalesOrderHeaderSalesReason
        SET
          SalesOrderHeaderSalesReason.ModifiedDate = GETUTCDATE()
      FROM Sales.SalesOrderHeaderSalesReason AS SalesOrderHeaderSalesReason
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = SalesOrderHeaderSalesReason.[Uuid]
      ;
