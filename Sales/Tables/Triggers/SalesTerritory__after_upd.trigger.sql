/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Sales].[TRG__SalesTerritory__after_upd]'))
DROP TRIGGER [Sales].[TRG__SalesTerritory__after_upd]
GO
*/
CREATE TRIGGER [Sales].[TRG__SalesTerritory__after_upd] ON [Sales].[SalesTerritory]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Sales
Object:      TRG__SalesTerritory__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__SalesTerritory__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditSalesTerritory]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    SELECT
      ...
      ,[AuditRecordsToSave].[CountryRegionCode]
      ,[AuditRecordsToSave].[Group]
      ,[AuditRecordsToSave].[SalesYTD]
      ,[AuditRecordsToSave].[SalesLastYear]
      ,[AuditRecordsToSave].[CostYTD]
      ,[AuditRecordsToSave].[CostLastYear]
    ...
    FROM
      ...
      UPDATE Sales.SalesTerritory
        SET
          SalesTerritory.ModifiedDate = GETUTCDATE()
      FROM Sales.SalesTerritory AS SalesTerritory
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = SalesTerritory.[Uuid]
      ;
