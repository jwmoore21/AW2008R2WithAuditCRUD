/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Sales].[TRG__Customer__after_upd]'))
DROP TRIGGER [Sales].[TRG__Customer__after_upd]
GO
*/
CREATE TRIGGER [Sales].[TRG__Customer__after_upd] ON [Sales].[Customer]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Sales
Object:      TRG__Customer__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__Customer__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditCustomer]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    FROM
      ...
      UPDATE Sales.Customer
        SET
          Customer.ModifiedDate = GETUTCDATE()
      FROM Sales.Customer AS Customer
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = Customer.[Uuid]
      ;
