/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Sales].[TRG__CreditCard__after_upd]'))
DROP TRIGGER [Sales].[TRG__CreditCard__after_upd]
GO
*/
CREATE TRIGGER [Sales].[TRG__CreditCard__after_upd] ON [Sales].[CreditCard]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Sales
Object:      TRG__CreditCard__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__CreditCard__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditCreditCard]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    SELECT
      ...
      ,[AuditRecordsToSave].[CardType]
      ,[AuditRecordsToSave].[CardNumber]
      ,[AuditRecordsToSave].[ExpMonth]
      ,[AuditRecordsToSave].[ExpYear]
    ...
    FROM
      ...
      UPDATE Sales.CreditCard
        SET
          CreditCard.ModifiedDate = GETUTCDATE()
      FROM Sales.CreditCard AS CreditCard
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = CreditCard.[Uuid]
      ;
