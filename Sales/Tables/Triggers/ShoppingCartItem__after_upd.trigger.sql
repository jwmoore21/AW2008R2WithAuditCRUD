/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Sales].[TRG__ShoppingCartItem__after_upd]'))
DROP TRIGGER [Sales].[TRG__ShoppingCartItem__after_upd]
GO
*/
CREATE TRIGGER [Sales].[TRG__ShoppingCartItem__after_upd] ON [Sales].[ShoppingCartItem]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Sales
Object:      TRG__ShoppingCartItem__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__ShoppingCartItem__after_upd';
    /* ========================================================================= */
    ...
    FROM
      ...
      UPDATE Sales.ShoppingCartItem
        SET
          ShoppingCartItem.ModifiedDate = GETUTCDATE()
      FROM Sales.ShoppingCartItem AS ShoppingCartItem
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = ShoppingCartItem.[Uuid]
      ;
