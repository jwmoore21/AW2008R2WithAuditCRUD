/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Sales].[TRG__Currency__after_upd]'))
DROP TRIGGER [Sales].[TRG__Currency__after_upd]
GO
*/
CREATE TRIGGER [Sales].[TRG__Currency__after_upd] ON [Sales].[Currency]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Sales
Object:      TRG__Currency__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__Currency__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditCurrency]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
