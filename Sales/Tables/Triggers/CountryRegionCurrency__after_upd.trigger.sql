/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Sales].[TRG__CountryRegionCurrency__after_upd]'))
DROP TRIGGER [Sales].[TRG__CountryRegionCurrency__after_upd]
GO
*/
CREATE TRIGGER [Sales].[TRG__CountryRegionCurrency__after_upd] ON [Sales].[CountryRegionCurrency]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Sales
Object:      TRG__CountryRegionCurrency__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__CountryRegionCurrency__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditCountryRegionCurrency]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    SELECT
      ...
      ,[AuditRecordsToSave].[CurrencyCode]
    ...
    FROM
      ...
      UPDATE Sales.CountryRegionCurrency
        SET
          CountryRegionCurrency.ModifiedDate = GETUTCDATE()
      FROM Sales.CountryRegionCurrency AS CountryRegionCurrency
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = CountryRegionCurrency.[Uuid]
      ;
