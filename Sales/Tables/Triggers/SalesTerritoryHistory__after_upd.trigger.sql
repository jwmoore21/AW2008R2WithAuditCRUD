/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Sales].[TRG__SalesTerritoryHistory__after_upd]'))
DROP TRIGGER [Sales].[TRG__SalesTerritoryHistory__after_upd]
GO
*/
CREATE TRIGGER [Sales].[TRG__SalesTerritoryHistory__after_upd] ON [Sales].[SalesTerritoryHistory]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Sales
Object:      TRG__SalesTerritoryHistory__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__SalesTerritoryHistory__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditSalesTerritoryHistory]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    FROM
      ...
      UPDATE Sales.SalesTerritoryHistory
        SET
          SalesTerritoryHistory.ModifiedDate = GETUTCDATE()
      FROM Sales.SalesTerritoryHistory AS SalesTerritoryHistory
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = SalesTerritoryHistory.[Uuid]
      ;
