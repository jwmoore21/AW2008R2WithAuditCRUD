/*
IF EXISTS ( SELECT 1 FROM sys.triggers WHERE object_id = object_id(N'[Sales].[TRG__PersonCreditCard__after_upd]'))
DROP TRIGGER [Sales].[TRG__PersonCreditCard__after_upd]
GO
*/
CREATE TRIGGER [Sales].[TRG__PersonCreditCard__after_upd] ON [Sales].[PersonCreditCard]
  AFTER UPDATE
AS

/*******************************************************************************
Schema:      Sales
Object:      TRG__PersonCreditCard__after_upd
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @_AuditOperation = 'Autosave by: AFTER UPDATE Trigger : TRG__PersonCreditCard__after_upd';
    /* ========================================================================= */
    ...
    /* Insert into a TEMP table, all of our deleted values */
    INSERT INTO [Audit].[AuditPersonCreditCard]
    (
       [AuditStatus]
      ,[AuditAppUser]
      ,[AuditSqlUser]
    ...
    FROM
      ...
      UPDATE Sales.PersonCreditCard
        SET
          PersonCreditCard.ModifiedDate = GETUTCDATE()
      FROM Sales.PersonCreditCard AS PersonCreditCard
        INNER JOIN @AuditRecordsToSave AS AuditRecordsToSave
        ON AuditRecordsToSave.[Uuid] = PersonCreditCard.[Uuid]
      ;
